...
| legacy-original | 8015 | REAL Legacy PHP form system (local legacy checkout, pinned a47ccba — see `tools/legacy-baseline/README.md`) |
| legacy-bootstrap | 8016 | Legacy-style Bootstrap 5 form demo (React) |
...
- **legacy-original**: [http://localhost:8015](http://localhost:8015)
- **legacy-bootstrap**: [http://localhost:8016](http://localhost:8016)
...
docker-compose logs -f legacy-original
docker-compose logs -f legacy-bootstrap
```
...
- **legacy-original**: curl check on port 80 (via Apache)
- **legacy-bootstrap**: HTTP check on port 80
