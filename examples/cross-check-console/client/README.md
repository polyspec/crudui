...
- `POST /api/validate` `{spec, data, files?, basepath?}` → 4-language v2 FORM
...
- `POST /api/render` `{spec, data, options}` → 3-framework v2 FORM SSR
...
- `POST /api/validate-list` `{listSpec, files?, basepath?}` → 4-language v2 list
  ...
  is `valid:true`.
- `POST /api/render-list` `{listSpec, rows, options}` → 3-framework v2 list SSR
...
- Collapsible doc panel: per-tab v2 syntax summary (form: role slots, condition
...
- Form examples quoted from `tests/fixtures/{validate,v2-render}/cases.json` and
  list examples from `tests/fixtures/v2-list-render/cases.json` (no invented
...
- Fixture export (per tab): serializes the current run into the matching
  ...
  `{name,note,spec,data,expected:{valid,errors}}`, v2-render
...
The console and the AI gate reach the SAME v2 engine functions
(`validateV2`, `renderFormV2`, `renderFormV2SSR`) through DIFFERENT call stacks:
