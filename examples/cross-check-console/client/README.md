...
- `POST /api/validate` `{spec, data, files?, basepath?}` → 4-language v2
...
- Collapsible doc panel: v2 syntax summary (role slots, condition maps,
...
- In-page doc panel.
- Example specs quoted from `tests/fixtures/{validate,v2-render}/cases.json`
...
- Fixture export: serializes current spec/data/options + results into the
  ...
  and v2-render `{name,note,spec,data,options,expected_html}` — and downloads
