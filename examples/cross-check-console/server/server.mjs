 ...
 *   (a) POST /api/validate — 4-language v2 validation fan-out (JS in-process;
 *       PHP/Go/Rust as stdin-JSON CLIs). Same v2 engine, four call stacks.
 *   (b) POST /api/render   — 3-framework v2 SSR (React/Svelte sync, Vue async),
 *       all in-process through the SAME v2 entries the conformance gate imports.
...
export async function handler(req, res) {
  ...
  // out across the four v2 CLIs in `mode:list` (compose → forbidden-scan; no
...
function startServer() {
  ...
  server.listen(PORT, async () => {
    // Warm the v2 engine (boot Vite SSR + load every v2 entry) before serving so
    // the first request does not pay the cold-start cost. A boot failure is fatal.
    process.stdout.write('Booting v2 engine (Vite SSR + 4-language wiring)...\n');
    try {
      await getEngine();
      process.stdout.write('v2 engine ready.\n');
    } catch (e) {
      process.stderr.write('FATAL: v2 engine boot failed: ' + (e && e.stack ? e.stack : e) + '\n');
      process.exit(1);
    }
