/**
 * Shared v2 RENDER engine loader (in-process). RENDER ONLY.
 *
 ...
 * validator and has zero privileged path. This engine loads ONLY the three v2
 * render adapters + the shared HTML normalizer.
 *
 * Why render stays in-process while validate does not: the Svelte v2 adapter
 ...
 * mode and `ssrLoadModule`s the exact same v2 render entries the AI conformance
 ...
 *
 *   - generator-react  src/v2/index.ts → renderFormV2     (sync)
 *   - generator-svelte src/v2/index.ts → renderFormV2     (sync)
 ...
 *
 * Vite resolution: the svelte v2 adapter needs @sveltejs/vite-plugin-svelte +
...
/**
 * Boot the Vite SSR environment and load every v2 render entry once. Idempotent:
...
async function bootEngine() {
  ...
  // stderr (the v2 React adapter emits raw HTML attribute names by design — its
