/**
 * 4-language v2 validate fan-out — ALL FOUR via stdin-JSON CLI subprocess.
 *
 ...
 *
 *   JS  : node --import tsx packages/validator-js/bin/validate-v2.mjs
 *   PHP : php packages/validator-php/bin/validate-v2.php       (cwd = pkg root)
 *   Go  : packages/validator-go/validate-v2                    (compiled)
 *   Rust: packages/validator-rust/target/release/validate-v2  (compiled)
 *
 * Shared contract:
 *   request : { spec, data, files?, basepath? }
 *   per lang: { lang, ok, valid, errors:[{path,field,rule,message,value}], ms, loadError }
 *
 * Each CLI runs the same v2 stack: compose (G5) → forbidden-scan (§6) → validate
 ...
 * a data-level validation failure (v2 core invariant; closes the v1
 * ProductNft.yml:873 gap).
 *
...
const JS_CLI = path.join(JS_PKG, 'bin/validate-v2.mjs');
const PHP_CLI = path.join(PHP_PKG, 'bin/validate-v2.php');
const GO_BIN = path.join(GO_PKG, 'validate-v2');
const RUST_BIN = path.join(RUST_PKG, 'target/release/validate-v2');
...
// --- The four v2 validate CLIs (JS symmetric with PHP/Go/Rust) ---------------

function runJs(payload) {
  // `node --import tsx` runs the .ts v2 source through the tsx loader (the same
  // validateV2 the conformance gate imports). tsx resolves from cwd = JS_PKG.
...
/**
 * Spawn a v2 validate CLI and normalize its stdout into the shared envelope.
 *
