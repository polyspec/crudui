...
for (const server of servers) {
  for (const mode of modes) {
    const file = `/data/${server}-${mode}-react.json`;
    let previous;
    try { previous = await readFile(file); } catch (error) { if (error.code !== 'ENOENT') throw error; }
    const url = action => `${base}/api/${server}/${action}/${mode}/react`;
    ...
    async function check(id, action) {
      ...
      results.push({ server, mode, id, passed: !error, ...(error ? { error } : {}) });
      process.stdout.write(`${server}/${mode}/${id}: ${error ? `FAIL ${error}` : 'PASS'}\n`);
    }
    try {
      for (const format of ['multipart', 'urlencoded', 'json']) {
        await check(`${format}/roundtrip`, async () => {
          ...
          assert.equal(saved.validatorSource, mode === 'original-keyed' ? 'original' : mode);
          ...
          if (equivalents.has(mode)) assert.deepEqual(signature, equivalents.get(mode), 'same result across servers and transports');
          else equivalents.set(mode, signature);
        });
        await check(`${format}/create-delete-create`, async () => {
          const before = await reset();
          const empty = mode === 'original' ? [] : {};
          const department = { name: '신규 부서 / New department' };
          const store = { name: 'New store', enabled: '', detail: 'A & B\n한글', title: { ko: '제목', en: 'Title' }, departments: mode === 'original' ? [department] : { [temporaryKey]: department } };
          const company = { name: 'New company', stores: mode === 'original' ? [store] : { [temporaryKey]: store } };
          if (mode === 'original') before.data.companies.splice(1, 0, company);
          else {
          ...
          if (mode !== 'original') assert.deepEqual(saved.keyChanges, [
          ...
          const inserted = await save({ companies: mode === 'original' ? [company] : { [temporaryKey]: company } }, format);
