...
try {
  ...
  const complete = report.reports.length === 24 * servers.length && variants.size === 24 * servers.length &&
    report.reports.every(result => result.results.length === 19) && report.interactions.length === 108 * servers.length && interactions.size === 108 * servers.length && report.interactions.every(result => servers.includes(result.server)) && report.initialMounts.length === 12 * servers.length;
