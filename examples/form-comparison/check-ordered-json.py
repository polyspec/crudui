...
def main():
    ...
    report = {'generatedAt': datetime.now(timezone.utc).isoformat(),
              ...
              'polyspecCommit': subprocess.check_output(['git', 'rev-parse', 'HEAD'], cwd=ROOT, text=True).strip(),
              ...
              'results': []}
    failed = False
    with tempfile.TemporaryDirectory(prefix='polyspec-json-cases-') as directory:
        cases = fixtures()
