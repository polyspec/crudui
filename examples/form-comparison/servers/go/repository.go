...
func loadData(state *object, mode string) (*object, error) {
	companies := []row{}
	for _, company := range sortedRows(storedRows(state, "companies"), "", "") {
		...
		for _, store := range sortedRows(storedRows(state, "stores"), "company", companySeq) {
			...
			for _, department := range sortedRows(storedRows(state, "departments"), "store", storeSeq) {
				...
				value := record("name", get(department, "name"))
				if mode == "original" {
					value.Set("department_seq", seq)
				}
				departments = append(departments, row{key, value})
			}
			key, err := sequenceKey(storeSeq)
			if err != nil {
				return nil, err
			}
			value := record("name", get(store, "name"), "enabled", get(store, "enabled"), "detail", get(store, "detail"), "title", clone(get(store, "title")), "departments", rowCollection(departments, mode))
			if mode == "original" {
				value.Set("store_seq", storeSeq)
			}
		...
		value := record("name", get(company, "name"), "stores", rowCollection(stores, mode))
		if mode == "original" {
			value.Set("company_seq", companySeq)
		}
		companies = append(companies, row{key, value})
	}
	return record("companies", rowCollection(companies, mode)), nil
}

func allocate(item row, existing []any, kind, mode string, next *object, seen map[string]bool) (string, error) {
	field := kind + "_seq"
	requested := ""
	if mode == "original" {
		var err error
		requested, err = text(get(item.value, field))
		if err != nil {
			return "", err
		}
	}
	...
	for _, raw := range existing {
		...
		if mode == "keyed" && key == item.key || mode == "original" && seq == requested {
			sequence = seq
			break
		}
	}
	if mode == "original" && requested != "" && sequence == "" {
		return "", inputError(fmt.Sprintf("Unknown or incorrectly owned %s: %s", field, requested))
	}
	if seen[sequence] {
...
func checkParent(key string, items []any, kind, parent, sequence, mode string) error {
	if mode != "keyed" {
		return nil
	}
	for _, raw := range items {
...
func (r repository) save(data *object, mode string) (*object, error) {
	return r.transaction(func(before *object) (*object, *object, error) {
		...
		change := func(path, key, sequence string) error {
			...
			if mode == "keyed" && saved != key {
				changes = append(changes, record("path", path, "oldKey", key, "newKey", saved))
			}
			return nil
		}
		companyRows, err := rows(get(data, "companies"), mode, "companies")
		if err != nil {
			return nil, nil, err
		}
		for _, company := range companyRows {
			companySeq, err := allocate(company, storedRows(before, "companies"), "company", mode, next, seenCompany)
			if err != nil {
				return nil, nil, err
			}
			companies = append(companies, record("company_seq", companySeq, "name", get(company.value, "name"), "position", len(companies)))
			storeRows, err := rows(get(company.value, "stores"), mode, "stores")
			if err != nil {
				return nil, nil, err
			}
			for i, store := range storeRows {
				if err = checkParent(store.key, storedRows(before, "stores"), "store", "company", companySeq, mode); err != nil {
					return nil, nil, err
				}
				storeSeq, err := allocate(store, owned(storedRows(before, "stores"), "company", companySeq), "store", mode, next, seenStore)
				if err != nil {
					return nil, nil, err
				}
				...
				departmentRows, err := rows(get(store.value, "departments"), mode, "departments")
				if err != nil {
					return nil, nil, err
				}
				for j, department := range departmentRows {
					if err = checkParent(department.key, storedRows(before, "departments"), "department", "store", storeSeq, mode); err != nil {
						return nil, nil, err
					}
					departmentSeq, err := allocate(department, owned(storedRows(before, "departments"), "store", storeSeq), "department", mode, next, seenDepartment)
					if err != nil {
						return nil, nil, err
					}
		...
		loaded, err := loadData(after, mode)
		if err != nil {
			return nil, nil, err
		}
