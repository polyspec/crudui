...
`crudui-validator` crate. Mirrors `examples/legacy/go-api`: same endpoints, same
...
```bash
docker compose -f examples/legacy/docker-compose.yml up rust-api
```

The service listens on host port `8017` (container `8080`) and mounts
`examples/legacy/shared-specs` at `/app/specs`.
