 ...
 * run it is to build the same .valid-target / data-rule-name form Legacy emits
 ...
 *     e.g. common[name], common[display][start_dt]  (legacy-client.validate.js:979-1025)
...
// ---------------------------------------------------------------------------
// Spec flattening: mirror legacy-client.validate.js fixSpec(), but also remember the
...
// Legacy renders these element kinds; the legacy runtime branches on
 ...
 *
 * name carries the bracket-path Legacy/legacy use; dotPath is the
...
function makeRealm() {
  ...
  // Globals legacy-client.validate.js references at top level.
...
// legacy-client.validate.js check() (lines 583-633) but capturing the method name.
...
function runLegacyCase(spec, input) {
  ...
  // (getPath, legacy-client.validate.js:748-788) has a level to strip for a single
  // leading dot. In real Legacy forms every field sits under the form's root
