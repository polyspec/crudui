 ...
 *   examples/legacy/limepie-original/assets/js/dist.validate.js
...
function flattenSpec(spec, bracketPrefix, dotPrefix, out) {
  ...
  for (const rawKey of Object.keys(properties)) {
    ...
    if (child && child.type === 'group') {
      ...
      // display_switch: the legacy dist.validate.js has no display-based validation
...
function makeRealm() {
  ...
  // (dist.validate.js:84, 265, 860). Silence them so output contains only results.
  const noop = () => {};
...
function runLegacyCase(spec, input) {
  ...
  if (unsupported.length) {
    ...
    const reasonMap = {
      ...
      'display_switch': 'display_switch field — legacy dist.validate.js has no display-based validation',
    };
