...
`validator-js` as the client replacement and never runs `legacy-client.validate.js`. If
...
`legacy-client.validate.js` is DOM-coupled (jQuery-validation style): `check()` reads the
...
1. Load `legacy-client.validate.js` in a jsdom realm with `$`, `jQuery`, `window`,
   `document`, `performance`, `console` bound (a stubbed `console` silences the
   file's stray `console.log(element)` calls).
2. Synthesize the Legacy form Legacy itself emits: one `.valid-target` element
...
- `display_switch`: **zero** references in `legacy-client.validate.js`. Display gating is
