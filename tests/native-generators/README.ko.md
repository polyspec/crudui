...
검사기는 JavaScript, PHP, Go, Rust, PHP 네이티브 확장을 실행합니다. 네이티브 모듈의
...
명세 멤버 순서와 컨트롤 속성 순서를 비교합니다. 기존 목록 사례 21개는 이미지
리소스 힌트를 포함한 원본 HTML 문자열을 비교합니다. 기존 정규화된 레이아웃
검사는 별도로 유지합니다.
...
`form`, `renderList`입니다. 성공 응답은 종료 상태 0을 사용합니다. 최상위 연산 오류는
`{ "error": { "code", "message", "at" } }`이며 종료 상태 1을 사용합니다. 폼 액션
실패는 해당 단계에 포함하고 실행을 계속합니다. 오류 메시지는 언어마다 다를 수
있지만 코드, 경로, 유지한 상태는 일치해야 합니다.
