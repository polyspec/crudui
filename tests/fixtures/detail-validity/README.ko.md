...
해석되지 않는 `$ref`는 메타 스키마를 통과하고 런타임에서 실패합니다. `$patch`로 들어온 금지 키는 둘 모두에서
실패합니다.
...
API는 `"pass"`에 대해 `{ valid: true, errors: [] }`에 해당하는 `Ok(())`를 반환합니다.
...
[상세 실행기 검사](../../../examples/cross-check-console/server/validate-detail-runner.test.mjs)와
