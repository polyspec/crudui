/**
 * Shared-fixture generator for the 4-language v2 validator (SPEC-V2 §2 pipeline).
 *
 * Runs the JS reference v2 validator for real and dumps, per case, the
 ...
 * it bit-for-bit (SPEC-V2 G-B 4-language idempotence: identical `valid`+errors).
 *
 ...
 *
 * `spec`   : a v2 root spec. Inline `{ type:'group', properties:{…} }`, or a
...
import {
  validateV2,
  ComposeLoadError,
} from '/Users/max/ai/gui/form-spec/packages/validator-ts/src/v2/validate/index';
...
const SPECS: CaseSpec[] = [
  ...
  // 20. design.show does NOT skip validation (SPEC-V2 R1 show/validate split).
  {
...
function build(c: CaseSpec): Record<string, unknown> {
  ...
  try {
    const result = validateV2(c.spec, c.data, c.files ? { files: c.files } : {});
