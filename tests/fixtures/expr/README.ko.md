...
TypeScript·PHP·Go·Rust는 `cases.json`을 불러옵니다. 각 사례는 표현식, 기대
토큰·AST, 기대 불리언·값 결과를 포함한 데이터 컨텍스트를 제공합니다.
...
경로 비교, 불리언 변환을 검사합니다. 빈 배열과 객체는 조건에서 참이며 필수
...
대조하고 네 언어 테스트를 모두 실행합니다. 실패한 구현을 통과시키기 위한
