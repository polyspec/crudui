...
컬렉션)과 값을 유지하는 숨긴 그룹도 검사합니다. 반복 사례
...
출력을 정규화합니다. 두 번째 스크립트는 위젯 사례와 [`written-cases.ts`](written-cases.ts)의 사례를
갱신하거나 추가합니다. 후자의 `expected_html`은 명세에서 작성하며 렌더링으로 만들지 않습니다.
