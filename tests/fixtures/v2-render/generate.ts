 ...
 *
 * Produces `cases.json`: one case per render scenario from the v2 analysis
 ...
 * of the React v2 reference generator (renderFormV2) — never hand-written. The
 * Vue/Svelte v2 generators must reproduce the same `expected_html` after the
...
const SCENARIOS: FixtureCase[] = [
  ...
  },
  {
    ...
    note: "v1 group_class exist/empty → design.group.class condition map { '.items': has-data, true: empty } → .form-group class branches on data.",
    spec: {
      type: 'group',
      properties: {
        block: {
