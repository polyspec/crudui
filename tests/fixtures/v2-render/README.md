# v2-render shared fixtures (3-framework parity gate)

`cases.json` is the single shared fixture for the v2 SSR renderer. All three
...
```jsonc
{
  ...
  "spec": { "type": "group", "properties": { /* v2 field specs */ } },
...
- v1 `group_class` exist/empty → `design.group.class` condition map (data-driven
...
`expected_html` is the React v2 reference generator's real output — never
hand-written. Regenerate (two steps: base, then appended widget cases):

```sh
# from repo root
node_modules/.bin/tsx tests/fixtures/v2-render/generate.ts > tests/fixtures/v2-render/cases.json
node_modules/.bin/tsx tests/fixtures/v2-render/gen-cases.mts
```

`generate.ts` imports `renderFormV2` from
`packages/generator-react/src/v2/index.ts`, renders every base scenario,
...
conformance test
(`packages/generator-react/src/__tests__/v2-render.conformance.test.ts`)
