...
check와 validate가 모두 성공하고 explain이 기획서와 일치할 때까지 반복한다. 단계를 건너뛰지 마라.
...
- **반복** "여러 개 / N개까지 / 추가·삭제 / 정렬" → `multiple: true` 또는 `multiple: { min, max, copy, sortable, onclick }`.
