---
name: nl-to-v2-form
description: 자연어 기획서/구술을 검증 통과하는 polyspec v2 스펙으로 변환한다. 사용자가 폼 설계서·필드 목록·요구사항을 주고 v2 스펙(YAML/JSON)을 요청할 때, 또는 기존 v2 스펙을 자연어 의도와 대조·수정할 때 사용한다.
---

# 자연어 → v2 form 생성

자연어 기획서/구술을 `polyspec check` + `validate`를 통과하는 polyspec v2 스펙으로 만든다.

## 0. 카탈로그는 코드가 가진다 — 여기 적지 마라 (강제)

위젯·layout·검증규칙·역할슬롯·구조버킷·금지키·표현식문법을 이 파일에 나열하지 마라. 항상
`polyspec describe`를 먼저 호출해 **현재** 카탈로그를 읽어라.

- 기계용: `polyspec describe --json` → `{ widgetCount, widgets[], layouts[], rules[], slots, buckets, forbiddenKeys, grammar, classification }`.
- 사람용: `polyspec describe --md`.
...
b. **분류** — describe의 `classification`(출처: `docs/SPEC-V2.md §3`)으로 각 키의 자리를 정한다:
...
d. **check** — `polyspec check <spec>`. 메타스키마(`additionalProperties: false`) + forbidden-scan.
   1급 외 키·금지키·미등록 슬롯키를 잡는다. RED면 c로 돌아가 고친다 — 표준을 낮춰 우회하지 마라.

e. **validate** — `polyspec validate <spec> <data> --lang all`. 4언어로 규칙 의미·표현식 평가를
   검증한다. mismatch(언어 간 불일치)나 비멱등이면 스펙이 틀린 것이다 — 고친다.

f. **미리보기** — `polyspec render <spec> --fw all` 또는 cross-check 콘솔. 3프레임워크 parity 확인.

g. **explain 역검증** — `polyspec explain <spec>`로 스펙을 자연어로 되돌려 기획서와 대조한다.
...
| 명령 | 용도 |
|---|---|
| `polyspec describe [--json\|--md]` | 코드/스키마 통합 카탈로그(위젯·layout·규칙·슬롯·노드·버킷·금지키·문법·분류). 초안 전 필수. |
| `polyspec check <spec>` | 메타스키마 + forbidden-scan. 1급 외 키·금지키·미등록 슬롯키 적발. |
| `polyspec validate <spec> <data> [--lang js\|php\|go\|rust\|all]` | 값 검증(규칙·표현식), 4언어 parity. |
| `polyspec render <spec> [--fw react\|svelte\|vue\|all]` | SSR 미리보기, 프레임워크 parity. |
| `polyspec explain <spec> [--lang ko\|en]` | 스펙 → 자연어 역검증. 기획서 대조용. |
| `polyspec scaffold [--type <widget>]` | describe 카탈로그 기반 최소 유효 골격(발명 0). |
| `polyspec list-widgets [--json]` | describe 위젯 섹션의 얇은 뷰. |

출처 단일진실: 카탈로그=코드(`packages/generator-core`·`packages/validator-ts`·`schema/polyspec-v2.schema.json`),
