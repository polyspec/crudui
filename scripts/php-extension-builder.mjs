...
async function pathState(filename) {
  try {
...
/** Require one absolute path composed only of regular directories and a final expected type. */
...
async function assertExecutable(filename, label) {
...
function pathDirectories(environment) {
  ...
  assert.ok(directories.length > 0, 'PATH must contain executable directories');
  assert.ok(directories.every(directory => path.isAbsolute(directory)),
    'PATH entries must be absolute');
  return [...new Set(directories)];
}

async function discoverExecutables(name, environment) {
  const candidates = [];
  for (const directory of pathDirectories(environment)) {
    ...
    if (!state) continue;
    assert.equal(state.isSymbolicLink(), false,
      'Discovered executable is a symbolic link: ' + candidate);
    assert.equal(state.isFile(), true,
      'Discovered executable is not a regular file: ' + candidate);
    await assertExecutable(candidate, name);
    candidates.push(candidate);
  }
  return candidates;
}

async function discoverOneExecutable(name, environment) {
...
async function verifyVersion(executable, pattern, run, environment) {
  const result = await run(executable, ['--version'], { capture: true, environment });
  assert.match(result.stdout.trim(), pattern,
    'Executable returned an unexpected version: ' + executable);
}

async function resolveExecutable(name, explicit, label, environment, run, versionPattern) {
...
async function resolveRustToolchain(explicitCargo, explicitRustc, environment, run) {
  if (explicitCargo || explicitRustc) {
    assert.ok(explicitCargo && explicitRustc,
      'Explicit Rust tools require both Cargo and rustc');
    const cargo = await assertExecutable(explicitCargo, 'Cargo');
    const rustc = await assertExecutable(explicitRustc, 'rustc');
    await verifyVersion(cargo, /^cargo /, run, environment);
    const version = await run(rustc, ['-vV'], { capture: true, environment });
    assert.match(version.stdout, /^rustc /, 'rustc returned an unexpected version');
    return { cargo, rustc, rustHost: rustHost(version.stdout) };
  }
  const rustup = await discoverOneExecutable('rustup', environment);
  ...
  const version = await run(records.rustc, ['-vV'], { capture: true, environment });
  assert.match(version.stdout, /^rustc /, 'rustc returned an unexpected version');
  return { ...records, rustHost: rustHost(version.stdout) };
}
...
export async function resolvePhpBuildTools(options = {}) {
  ...
  const rust = await resolveRustToolchain(options.cargo, options.rustc, environment, run);
