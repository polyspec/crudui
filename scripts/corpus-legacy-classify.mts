...
import { translateFromLegacy } from '../packages/validator-js/src/legacy/translate/index';
import { scanForbiddenKeys } from '../packages/validator-js/src/forbidden-scan';
import { validate } from '../packages/validator-js/src/validate/index';
import { ComposeLoadError } from '../packages/validator-js/src/compose/index';
import type { FileLoader } from '../packages/validator-js/src/compose/index';
...
type Cat = 'corpus-data' | 'translator-bug' | 'schema-gap';
function classify(stage: string, reason: string, ctx: { errKw?: string; ap?: string; iPath?: string; surviving?: string }): { cat: Cat; label: string } {
  ...
  if (/\/items(\/|$)/.test(ip)) {
    return { cat: 'schema-gap', label: 'items static map with LangMap labels — schema Items lacks {value:LangMap} branch' };
  }
  if (/\/design(\/|$)/.test(ip)) {
    return { cat: 'translator-bug', label: 'design.class/style emitted as {} on null condition-class merge' };
  }
  if (/\/behavior(\/|$)/.test(ip)) {
    return { cat: 'translator-bug', label: 'behavior.on* boolean flag passed through — schema BehaviorAction expects script' };
  }
  if (kw === 'additionalProperties') {
    return { cat: 'schema-gap', label: `unmodeled surviving key +${ctx.ap}` };
  }
  if (kw === 'not' || kw === 'propertyNames') {
    return { cat: 'translator-bug', label: 'forbidden key name survived into schema (x-key etc.)' };
  }
  return { cat: 'schema-gap', label: `schema ${kw} @${ip}` };
}
...
for (const file of list) {
  ...
  let legacy: any;
  try { legacy = yaml.load(fs.readFileSync(file, 'utf8')); } catch (e: any) {
  ...
  let schema: any;
  try { schema = translateFromLegacy(legacy as any).schema; translated++; } catch (e: any) {
  ...
  try { scanForbiddenKeys(schema, []); } catch (e: any) {
  ...
  try { validate(schema, {}, { loader: refLoader, basepath: path.dirname(file) }); } catch (e: any) {
...
fs.writeFileSync(path.join(REPO, 'scripts/corpus-legacy-classified.json'), JSON.stringify(out, null, 2));
