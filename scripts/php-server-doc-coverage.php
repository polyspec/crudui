 ...
 */
function polyspec_php_server_doc_gaps(string $file): array
{
 ...
 */
function polyspec_php_server_files(): array
{
...
if (PHP_SAPI === 'cli' && isset($argv[0]) && realpath($argv[0]) === realpath(__FILE__)) {
    $files = polyspec_php_server_files();
    if (count($files) === 0) {
    ...
    foreach ($files as $file) {
        $gaps = array_merge($gaps, polyspec_php_server_doc_gaps($file));
    }
    sort($gaps);
