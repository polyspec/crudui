...
function genRust() {
  log('Rust: cargo doc --no-deps -p crudui-validator -> target/doc');
  ...
  try {
    ...
    execSync('cargo doc --no-deps -p crudui-validator', {
      cwd: rustPkgDir,
      env,
      stdio: 'inherit',
    });
    docPath = join(rustPkgDir, 'target', 'doc', 'crudui_validator', 'index.html');
    ok = existsSync(docPath);
  } catch (e) {
    warn(`cargo doc failed: ${e.message}`);
  }
  const rel = 'packages/validator-rust/target/doc/crudui_validator/index.html';
  const md =
    '# Rust API — crudui-validator\n\n' +
    'Crate: `crudui-validator`\n\n' +
    (ok
      ? 'Rustdoc HTML is generated by `cargo doc --no-deps -p crudui-validator` into\n' +
...
function genPhp() {
  ...
  if (!runner) {
    ...
    writeFileSync(
      outFile,
      '# PHP API — crudui/validator\n\n' +
        'Package: `crudui/validator` (namespace `CRUDUI\\Validator`, PHP ^8.2)\n\n' +
  ...
  try {
    ...
    execSync(
      `${runner} run -d ${JSON.stringify(join(phpPkgDir, 'src'))} -t ${JSON.stringify(outDir)} ` +
        `--cache-folder ${JSON.stringify(cacheDir)} --title "crudui/validator" --no-interaction`,
...
if (target === 'all') {
  ...
  const index =
    ...
    tsLinks +
    '\n\n## Go\n\n- [validator-go](./go.md)\n\n' +
    '## Rust\n\n- [crudui-validator](./rust.md)\n\n' +
    '## PHP\n\n- [crudui/validator](./php.md)\n';
