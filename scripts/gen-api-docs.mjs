...
const TS_PACKAGES = [
  {
    ...
    entry: 'src/index.ts',
    out: 'generator-core',
    title: '@polyspec/generator-core',
  },
  {
    pkg: 'validator-ts',
    entry: 'src/index.ts',
    out: 'validator-ts',
    title: '@polyspec/validator (validator-ts)',
  },
  {
    pkg: 'generator-react',
    entry: 'src/index.ts',
    out: 'generator-react',
    title: '@polyspec/generator-react',
  },
  {
    pkg: 'generator-vue',
    entry: 'src/index.ts',
    out: 'generator-vue',
    title: '@polyspec/generator-vue',
  },
  {
    ...
    title: '@polyspec/generator-svelte (TypeScript helpers)',
...
function genRust() {
  log('Rust: cargo doc --no-deps -p polyspec-validator -> target/doc');
  ...
  try {
    ...
    execSync('cargo doc --no-deps -p polyspec-validator', {
      cwd: rustPkgDir,
      env,
      stdio: 'inherit',
    });
    docPath = join(rustPkgDir, 'target', 'doc', 'polyspec_validator', 'index.html');
    ok = existsSync(docPath);
  } catch (e) {
    warn(`cargo doc failed: ${e.message}`);
  }
  const rel = 'packages/validator-rust/target/doc/polyspec_validator/index.html';
  const md =
    '# Rust API — polyspec-validator\n\n' +
    'Crate: `polyspec-validator`\n\n' +
    (ok
      ? 'Rustdoc HTML is generated by `cargo doc --no-deps -p polyspec-validator` into\n' +
...
function genPhp() {
  ...
  if (!runner) {
    ...
    writeFileSync(
      outFile,
      '# PHP API — polyspec/validator\n\n' +
        'Package: `polyspec/validator` (namespace `Polyspec\\Validator`, PHP ^8.2)\n\n' +
  ...
  try {
    ...
    execSync(
      `${runner} run -d ${JSON.stringify(join(phpPkgDir, 'src'))} -t ${JSON.stringify(outDir)} ` +
        `--cache-folder ${JSON.stringify(cacheDir)} --title "polyspec/validator" --no-interaction`,
...
if (target === 'all') {
  ...
  const index =
    ...
    tsLinks +
    '\n\n## Go\n\n- [validator-go](./go.md)\n\n' +
    '## Rust\n\n- [polyspec-validator](./rust.md)\n\n' +
    '## PHP\n\n- [polyspec/validator](./php.md)\n';
