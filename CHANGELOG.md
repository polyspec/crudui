...
- `tests/fixtures/specs/ProductNft.yml` had two duplicate YAML keys, which the benchmark fixture
...
The legacy form components and the original Limepie rendering comparisons were
...
- `examples/legacy/demo-app`, `playground`, `limepie-bootstrap`, `limepie-compare`,
  `limepie-original`, `limepie-validate-test` and `react-usage.tsx`, with their
  docker-compose services and README entries;
- `tests/parity`, `tests/cross-framework`, `tests/legacy-client`,
  `tests/fixtures/reference-html`, `tools/limepie-baseline`, the root `compare`
