...
- **Compare down a column, within one spec.** The four rows of a spec are the
  ...
  `productnft` runs the empty form, which **short-circuits at the first
...
- **The empty-form productnft input is deliberate.** The raw Limepie
  `ProductNft.yml` carries literal `[]`-suffixed keys for its `multiple` fields
