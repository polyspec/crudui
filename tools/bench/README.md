...
| `productnft` | 80 current-schema fields | empty form `{}`  | generated by `gen-fixtures.js` |
...
- **The empty-form productnft input is deliberate.** `gen-fixtures.js` generates
  an 80-field current-schema specification and an empty object, so every backend
  reaches the same required-field result without depending on a legacy schema or
  a backend-specific normalization rule.
