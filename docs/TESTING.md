...
| HTML parity (React) | `tests/parity/parity.test.mjs` | `cd tests/parity && npm test` | 기준 HTML 7종 | React SSR ↔ Legacy 기준 HTML (7/7) |
| HTML parity (Vue) | `packages/generator-vue/test/parity.test.mjs` | `cd packages/generator-vue && npm test` | 기준 HTML 7종 | Vue SSR ↔ Legacy 기준 HTML (7/7) |
| HTML parity (Svelte) | `packages/generator-svelte/test/parity.test.mjs` | `cd packages/generator-svelte && npm test` | 기준 HTML 7종 | Svelte SSR ↔ Legacy 기준 HTML (7/7) |
...
| legacy 클라이언트 비교 | `tests/legacy-client/gate.js` | `cd tests/legacy-client && npm run gate` | 1074 (비교 가능분) | jQuery legacy-client.validate.js ↔ 새 검증기 (jsdom) |
...
│   └── generator-legacy/    # legacy Legacy vendor 체크아웃 + web assets (기준 파이프라인 지원)
├── tests/
│   ├── cases/               # 크로스 언어 픽스처 23개 (1074 케이스) — 단일진실
│   ├── fixtures/
│   │   ├── reference-html/     # Legacy PHP 기준 HTML 7종 (재생성: tools/legacy-baseline)
...
│   └── legacy-client/       # legacy jQuery legacy-client.validate.js ↔ 새 검증기 (jsdom)
└── tools/
    ├── legacy-baseline/    # 기준 HTML 재생성 파이프라인 (핀 커밋 강제)
...
`tests/fixtures/reference-html/*.html`(legacy Legacy PHP 출력, 단일진실)과
...
비교 대상: `examples/legacy/shared-specs/*.yml` 6종 + `tests/fixtures/specs/LargeForm.yml`.
모두 빈 데이터 렌더 기준. 세 프레임워크 모두 기준 HTML 7종에 7/7 GREEN 이다.

## 5. 기준 HTML 재생성 (tools/legacy-baseline)
...
# 3. 단일 스펙 렌더 (확인용)
php tools/legacy-baseline/render.php examples/legacy/shared-specs/product-form.yml
```
...
legacy Legacy 의 jQuery 브라우저 검증기 `examples/legacy/legacy-original/assets/js/
legacy-client.validate.js` 를 jsdom + jquery 로 구동해 새 검증기와 비교한다. legacy 폼이
...
3종(required 공백 미트림,
암묵 number 미강제, malformed min/max 임계값)은 `legacy-client.validate.js` 를 새 검증기
