...
Repository SSR helpers are `renderFormV2` in `src/v2/index.ts` for React and
Svelte, and `renderFormV2SSR` in `src/v2/ssr.ts` for Vue. These source helpers are
not package subpath exports. Compile `$ref` files before rendering.

Validate `submission` with `ValidatorV2` from `@polyspec/validator` and the original
