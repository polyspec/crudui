...
두 명령 모두 조용히 기다리지 않습니다. 모든 단계는 자기 제한을 포함한 시작, 실행 중
15초마다 경과 시간, 완료 시 소요 시간을 출력합니다. 한 작업인 단계는 제한 시간을 가지고,
트리 검증 자체나 브라우저 검사, 정본 흐름 검사처럼 단위로 이루어진 단계는 전체 제한 시간 없이
단위 진행 줄이 45초 동안 없을 때 중단됩니다. 어느 제한에든 도달한 단계는 프로세스 트리
...
현재 빌드 주기를 단위 하나로 기다리며 제한 시간 10분 안에서 15초마다 경과 시간을 출력하고 ready를
...
제한 시간에 도달하거나 45초 동안 진행 줄이 없을 때만 실패합니다. 실패·누락·잘못된
