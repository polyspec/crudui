...
>   (`tools/limepie-baseline/`)과 SSR 비교 하네스. React 는 `tests/parity/`,
...
| 패키지 | 상태 |
|--------|------|
| @crudui/validator (JS) | legacy.0.0 완성 |
| validator-php | 완성 (PHP ^8.2) |
| validator-go | 완성 |
| validator-rust (crudui-validator) | 완성 |
| @crudui/generator-react | 완성 — 기준 HTML 7/7 parity (353 테스트) |
| @crudui/generator-vue | 완성 — 기준 HTML 7/7 parity (@vue/server-renderer SSR) |
| @crudui/generator-svelte | 완성 — 기준 HTML 7/7 parity (Svelte 5 SSR) |
...
프레임워크 무관 PHP-cast 헬퍼 `src/limepieParity.ts` 를 공유 패턴으로 쓴다.
...
(`packages/validator-ts/src/legacy/Validator.ts` `evaluateCondition`, 776-791행).
...
| **CRUDUI** | JS/PHP/Go/Rust | 보장 | 높음 | **유일한 적합** |
...
| # | 작업 | 파일 | 담당 |
|---|------|------|------|
| 1 | 에러 처리 개선 - Validator | `packages/validator-ts/src/legacy/Validator.ts` | Agent 1 |
...
| 12 | CLI 도구 | `crudui validate`, `crudui generate` | DX 향상 |
| 13 | VS Code 확장 | YAML 스펙 자동완성, 문법 검사 | DX 향상 |
| 14 | 문서 사이트 | VitePress/Docusaurus 기반 문서 | 채택률 향상 |

~~OpenAPI 스키마 생성~~ - 제외 (CRUDUI이 OpenAPI보다 상위 개념, 변환 시 60% 정보 손실)
