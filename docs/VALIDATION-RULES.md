...
polyspec 검증기에 **실제 등록된** 규칙의 목록과 동작 명세.
...
(`packages/validator-php/src/Validator.php:48-73`).
...
2. **빈 값 스킵** — `required`를 제외한 규칙은 값이 비어 있으면(null / 빈 문자열 /
   ...
   mincount 위반인 경우 — `packages/validator-php/src/Validator.php:370-375`,
...
- JS: `ARRAY_LEVEL_RULES` — `packages/validator-ts/src/Validator.ts:132`
...
- 새 검증기 출처: `packages/validator-ts/src/Validator.ts:629-652`
...
- 새 검증기 출처: `packages/validator-ts/src/Validator.ts:293-296`(객체 multiple
...
| 규칙명 | 상태 |
|--------|------|
| `minformcount` | 미구현. 픽스처·ProductNft 스펙 어디에도 미사용. JS `RulesSpec` 타입 선언(`types.ts:109`)에만 존재하며 구현체 없음 |
| `maxformcount` | 미구현. 동상 |
