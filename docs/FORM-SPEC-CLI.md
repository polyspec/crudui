...
`form-spec` CLI 는 자연어 → CRUDUI 폼 스펙 작성 루프의 도구층이다. 자체 엔진을 갖지 않는다 — 코드/스키마 단일진실을 import/parse 하거나, 콘솔의 4언어 게이트웨이를 호출하는 얇은 래퍼다. 검증·렌더 로직을 CLI 에 재구현하지 마라.

- 패키지: `packages/form-spec-cli` (pkg 명 `@form-spec/cli`, bin: `form-spec`)
- 실행: `tsx` 로더로 TS 소스 직접 실행 (`validate.mjs` 와 동일 방식, 별도 빌드 없음).

**구현 vs 로드맵 (정직 분리)**. 이 문서는 두 상태를 섞지 않는다.

- **구현됨**: `describe` · `check` · `explain` · `list-widgets`. `bin/form-spec.mjs` 에 등록돼 동작한다. 소스: `src/describe.ts` · `src/check.ts` · `src/explain.ts` (list-widgets 는 describe 의 얇은 뷰, 별도 src 없음).
- **로드맵 (미구현)**: `validate` · `render` · `scaffold`. `src/` 에 파일 없음, `bin/form-spec.mjs` 에 미등록 — 지금 호출하면 `unknown subcommand` 로 종료(exit 2)한다. 콘솔 백엔드 위임 설계만 확정, 코드는 아직 없다.
...
`bin/form-spec.mjs` USAGE 가 실제 노출하는 명령은 구현된 4개(`describe`/`list-widgets`/`check`/`explain`)뿐이다. 로드맵 3개는 `default` 분기에서 `unknown subcommand`(exit 2)로 떨어진다.
...
| 규칙 분류 (파라미터 의미) | `packages/validator-js/src/validate/validator.ts` `ARRAY_LEVEL_RULES` `PATH_REFERENCE_RULES` `LITERAL_PARAM_RULES` `REGEX_PARAM_RULES` `MEMBERSHIP_PARAM_RULES` + `type:number` 암묵 number 규칙 |
| 슬롯/구조/버킷 | `schema/form-spec.schema.json` definitions(`Field`·`Validate`·`Design`·`Behavior`·`Options`·`Items`·`ItemsSource`·`ItemsModel`·`Multiple`·`Lang`·`Properties`)를 JSON.parse |
| design 노드 이름 | `packages/validator-js/src/schema.ts` `DesignNodeName` union(`show`/`class`/`style`/`label`/`wrapper`/`group`/`prepend`) + schema `Design` |
...
| list 구조 | `schema/form-spec.schema.json` definitions(`List`·`Column`·`CellFormat`·`Pagination`·`Sort`·`ListAction`)를 JSON.parse — schema §9 |
...
```json
{
  ...
  "classification": { "source": "docs/spec/schema.md §3", "firstClass": { "structure": ["..."], "content": ["..."], "roleSlots": ["validate", "design", "behavior", "options"] }, "dependencyIsolation": [{ "trigger": "...", "target": "...", "note": "..." }], "roleDistribution": [{ "role": "...", "target": "..." }] },
...
1. **메타스키마** — ajv vs `schema/form-spec.schema.json` (`additionalProperties:false`, `required:[type]`). 1급외 키 / 미등록 슬롯키 / `ForbiddenKeyNames` 적발.
2. **forbidden-scan** — `scanForbiddenKeys` (validator-js/forbidden-scan.ts), 임의 깊이. 메타스키마가 거울로 가진 런타임 백스톱.
...
> 상태: `src/validate.ts` 없음, `bin/form-spec.mjs` 미등록. 지금 `form-spec validate ...` 는 `unknown subcommand`(exit 2). 아래는 확정 설계다. 콘솔의 4언어 게이트웨이(`validate-runner.mjs`)와 4언어 CRUDUI CLI(`validator-js/bin`·`php/bin`·`go/cmd/validate`·`rust/src/bin`)는 이미 구현·동작하지만, 그것을 호출하는 form-spec CLI 래퍼는 아직 없다.
...
- JS: `node --import tsx packages/validator-js/bin/validate.mjs` (stdin JSON)
- PHP: `php packages/validator-php/bin/validate.php`
- Go: `packages/validator-go/validate` (컴파일)
- Rust: `packages/validator-rust/target/release/validate` (컴파일)

각 CLI 는 동일 CRUDUI 스택을 돈다: compose(G5) → forbidden-scan(§6) → validate(§3 + §2 G1).
...
> 상태: `src/render.ts` 없음, bin 미등록 (`unknown subcommand`, exit 2). 아래는 확정 설계다. 콘솔 `render-runner.mjs renderAll` 과 3프레임워크 SSR(`engine.mjs`)은 이미 구현·동작하지만, 그것을 호출하는 form-spec CLI 래퍼는 아직 없다.
...
```bash
form-spec scaffold --type email   # 로드맵 — 현재 unknown subcommand
```
...
## 3. describe 구현 노트 (`packages/form-spec-cli/src/describe.ts` — 구현됨)
...
3. **슬롯/구조/버킷** — `JSON.parse(schema/form-spec.schema.json)` → `Field.properties`(1급), `Validate`/`Design`/`Behavior`/`Options`/`Items`/`ItemsSource`/`ItemsModel`/`Multiple`/`Lang` definitions, `DesignNode`, `ForbiddenKeyNames` enum.
4. **금지키** — `import { FORBIDDEN_META_KEYS, FORBIDDEN_META_KEY_PATTERN } from '../../validator-js/src/schema.ts'` + `scanForbiddenKeys`. types.ts enum ≡ schema enum ≡ 런타임 scan 3중 cross-check — 불일치 시 `crossCheckOk:false`.
...
7. **분류** — `spec/schema.md` §3 A/B/C 규칙 인용.

## 4. 콘솔 재사용 경계 (validate/render 는 로드맵)

form-spec-cli 는 오케스트레이터다. 어떤 검증/렌더 로직도 CLI 에 재구현하지 않는다 — 구현 시 콘솔 백엔드를 위임한다.
