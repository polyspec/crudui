...
메타스키마 검증(`schema/form-spec.schema.json`, `additionalProperties:false`, `required:[type]`) + forbidden-scan(`scanForbiddenKeys`, 임의 깊이). 1급외 키 / 금지키 / 미등록 슬롯키를 정적으로 적발한다. 값 검증은 하지 않는다 — 그건 `validate` 다.
...
3. **슬롯/구조/버킷** — `JSON.parse(schema/form-spec.schema.json)` → `Field.properties`(1급), `Validate`/`Design`/`Behavior`/`Options`/`Items`/`Multiple`/`Lang` definitions, `DesignNode`, `ForbiddenKeyNames` enum.
...
6. **분류** — `spec/schema.md` §3 A/B/C 규칙 인용.
