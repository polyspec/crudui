...
crudui MCP 서버는 LLM 이 자연어 → CRUDUI 폼 스펙을 작성할 때 카탈로그 로드·정적 검증·값 검증·렌더·역검증을 즉시 호출하는 도구 묶음이다. 모든 tool 은 `crudui` CLI 명령에 위임한다 — MCP 는 별도 엔진을 갖지 않는다. CLI 가 코드/스키마 단일진실(`describe`/`check`)과 콘솔 4언어 게이트웨이(`validate`/`render`)를 재사용하므로, MCP 도 그 단일진실/게이트웨이를 그대로 위임받는다.

**구현 상태 (정직 분리)**. **MCP 서버 자체는 아직 미구현이다 — 이 문서는 tool 표면 설계(스펙)다.** 저장소에 MCP 서버 코드는 없다(`docs/CRUDUI-MCP.md` 가 유일한 산출물). tool 별 위임 대상의 실현 가능성은 위임하는 CLI 명령 상태를 따른다:

- **위임 CLI 가 구현됨**: `crudui.describe`(→ CLI `describe`) · `crudui.check`(→ `check`) · `crudui.explain`(→ `explain`) · `crudui.schema`(스키마 원본 반환). MCP 서버만 붙이면 즉시 동작 가능.
- **위임 CLI 가 로드맵**: `crudui.validate`(→ CLI `validate`, 미구현) · `crudui.render`(→ CLI `render`, 미구현). CLI 래퍼가 먼저 구현돼야 한다.

상세 IO·재사용 경계·구현 노트는 `docs/CRUDUI-CLI.md` 참조. 이 문서는 tool 표면과 입력/출력 schema 만 정의한다.

## 0. 위치

```
LLM  ──(자연어→스펙 루프)──▶  MCP tools  ──(위임)──▶  crudui CLI
...
LLM 은 초안 전 `crudui.describe` 로 현재 카탈로그를 로드하고, 초안 후 `crudui.check`(정적) → `crudui.explain`(역검증) 으로 루프를 닫는다. `crudui.validate`(4언어) → `crudui.render`(3프레임워크) 단계는 위임 CLI(`validate`/`render`)가 구현되면 합류한다(로드맵).

## 1. tool 요약

| tool | 위임 CLI | 한 줄 | 위임 CLI 상태 |
|---|---|---|---|
| `crudui.describe` | `describe` | 통합 카탈로그 (위젯·규칙·슬롯·금지키·문법·분류 + **list capability**) | **구현됨** |
| `crudui.check` | `check` | 메타스키마 + forbidden-scan + leaf-type 정적 검증 | **구현됨** |
| `crudui.explain` | `explain` | 스펙 → 자연어 역검증 | **구현됨** |
| `crudui.schema` | (없음) | `schema/crudui.schema.json` 원본 메타스키마 | **구현됨** (파일 반환) |
| `crudui.validate` | `validate` | 값 검증 (규칙·표현식, 4언어) | **로드맵** (CLI 미구현) |
| `crudui.render` | `render` | SSR 미리보기 (3프레임워크) | **로드맵** (CLI 미구현) |
...
### 2.1 `crudui.describe` (위임 CLI 구현됨)
...
### 2.3 `crudui.validate` (로드맵 — 위임 CLI 미구현)
...
### 2.4 `crudui.render` (로드맵 — 위임 CLI 미구현)
...
### 2.6 `crudui.schema` (구현됨 — 파일 반환)

CLI 위임 아님 — `schema/crudui.schema.json` 원본을 그대로 반환한다. describe 는 이 메타스키마에서 파생, schema 는 원본이다. AI 가 ajv 로 직접 검증할 때 사용. 메타스키마는 form(`Field`)과 list(`List`/`Column`/`CellFormat`/…) definitions 를 모두 포함한다.
...
로드맵 합류 (위임 CLI 구현 후): 3 과 4 사이에 `crudui.validate`(4언어 값 검증) → `crudui.render`(3프레임워크 미리보기)가 들어간다. 그때까지 값 검증·렌더는 콘솔(`examples/cross-check-console`)·CLI(`validate`)로 직접 돌린다.
