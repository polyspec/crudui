...
`display_switch` / `display_target`의 정본(canonical) 의미론.
두 속성 모두 **숨김 대상 필드 자신에** 선언하며, 4개 언어 검증기
(JS/PHP/Go/Rust)가 동일하게 평가한다. 숨겨진 필드는 **검증이 스킵된다**.

검증기 구현 출처:

- JS: `packages/validator-js/src/Validator.ts` `shouldValidateField()` (L395-448)
- PHP: `packages/validator-php/src/Validator.php` `shouldValidateField()` (L441-473)
- Go: `packages/validator-go/validator/validator.go` `shouldDisplay()` (L361-407)
- Rust: `packages/validator-rust/src/validator.rs` `should_display()` (L421-)
...
> 정본 형식은 아래와 같이 대상 필드 자신에 붙는 조건식 문자열 또는 boolean이다.
...
구현(예시는 React): `packages/generator-react/src/hooks/useConditional.ts`
(`evaluateAllOf`), `packages/generator-react/src/components/FormField.tsx`.
타입: `packages/generator-react/src/types.ts` (`AllOfCondition`, `ElementConfig`).
Vue/Svelte 는 각각 `packages/generator-vue/src/hooks/legacyDisplay.ts`,
`packages/generator-svelte/src/legacyDisplay.ts` 에 동일 의미론을 둔다.
