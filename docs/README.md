...
3. **Legacy PHP 호환**: 기존 Legacy 시스템의 폼 스펙과 완전 호환
4. **복잡한 폼 지원**: 1,318줄 규모의 LargeForm/Spec/Create.yml 같은 복잡한 e-commerce 폼 완벽 지원
...
이 프로젝트의 주요 목표는 **LargeForm/Spec/Create.yml** (1,318줄) 같은 복잡한 e-commerce 폼을 완벽히 지원하는 것입니다.
...
- [마이그레이션 가이드 (Legacy)](./migration-from-legacy.md)
