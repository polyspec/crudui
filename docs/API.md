...
polyspec 검증기의 4개 언어 구현(JavaScript/TypeScript, PHP, Go, Rust) 공개 API 레퍼런스.
...
- [공통 계약](#공통-계약)
- [JavaScript/TypeScript (`@polyspec/validator`)](#javascripttypescript-polyspecvalidator)
- [PHP (`polyspec/validator`)](#php-polyspecvalidator)
- [Go (`validator` 패키지)](#go-validator-패키지)
- [Rust (`polyspec-validator` 크레이트)](#rust-polyspec-validator-크레이트)
...
## PHP (`polyspec/validator`)

출처: `packages/validator-php/src/Validator.php`, `packages/validator-php/src/ValidationResult.php`
...
- `Polyspec\Validator\ConditionParser` — 조건식 평가
  ...
  출처: `packages/validator-php/src/ConditionParser.php`
- `Polyspec\Validator\PathResolver` — 경로 해석
...
Go 검증기의 내부 `Spec`은 필드 슬라이스 구조다. 정본(canonical) JSON 스펙
(`type`/`properties` 형식)은 `ParseSpec`으로 변환한다. 이때 property·rule
선언 순서가 보존된다.

```go
// ParseSpec parses a canonical polyspec JSON document
...
크레이트: `polyspec-validator` (`packages/validator-rust/Cargo.toml`).
...
```rust
use polyspec_validator::{parse_spec, ParsedSpec, Validator};
