...
외부에 보존한 `localhost:4317` 환경은 라이브러리 `83181c2`을
...
| PHP 확장 | `2dfd321` | 120/120 통과 | 30/30 통과 | 0 |
| Go | `2dfd321` | 120/120 통과 | 30/30 통과 | 0 |
| Rust | `2dfd321` | 120/120 통과 | 30/30 통과 | 0 |
...
`.form-comparison/results/report-<server>.json`에 저장하며 이전 리비전의 결과는
`results-2dfd321/`에 보존합니다. 보고서 메타데이터는 소스 리비전을 기록합니다. 상호작용 검사 전체 108개는 비교 모드 네 가지의
...
`83181c2`의 PHP 브라우저 검사가 완료되었으며 나머지 서버는 재검증 중입니다. 보존된 비교 구현의 실패는 보고서에 유지합니다.
