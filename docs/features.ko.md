...
현재 `main` 트리가 비교 배포에서 제공되고 있습니다. 컨테이너 재시작 후 배포 healthcheck와
supervisor가 ready 상태에 도달했습니다. PHP, PHP 확장, Go, Rust가 생성
...
정본 목록은 45개 레코드를 20·20·5개 세 페이지로 제공합니다. 페이지 수준 SSR 소스에는
선택한 CRUDUI 목록 또는 상세 마크업이 포함되고 CSR 소스에는 단계 셸만 포함됩니다. 현재
트리의 전체 브라우저 검증은 검증과 소스 식별자 실패로 종료되었으므로 통과로 기록하지
않으며, 위의 과거 무실패 결과는 현재 트리에 적용되지 않습니다.
