...
features:
  ...
  - title: v2 표현식 엔진
    details: 조건을 값에 녹인다 — 제한 DSL(eval 금지)을 4언어가 같은 토큰열·같은 AST·같은 값으로 평가(멱등). 조건맵으로 분기, 조건 전용 메타키 0. (SPEC-V2 §6 은 display_switch/display_target 등 조건 전용 메타키를 금지한다.)
  - title: v2 도구체인
    details: polyspec CLI(describe/check/explain/list-widgets) · MCP · 크로스-검증 콘솔(4언어 검증 × 3프레임워크 SSR) · nl-to-v2-form 스킬. 모두 코드/스키마 단일진실에 위임한다.
  - title: 기계가독 스펙
...
Polyspec 은 YAML 한 벌로 폼의 구조·검증 규칙·조건부 표시를 정의하고, 이를
...
| 영역 | 패키지 | 비고 |
|------|--------|------|
| 검증기 | `validator-ts` (`@polyspec/validator`) | TypeScript, 브라우저·Node |
| 검증기 | `validator-php` (`polyspec/validator`) | PHP ^8.2 |
| 검증기 | `validator-go` | `github.com/polyspec/polyspec/packages/validator-go` |
| 검증기 | `validator-rust` (`polyspec-validator`) | Rust 크레이트 |
| 렌더 코어 | `generator-core` (`@polyspec/generator-core`) | 프레임워크 무관 buildForm/buildList |
| 렌더러 | `generator-react` (`@polyspec/generator-react`) | 기준 HTML 7/7 parity, list ListV2 |
| 렌더러 | `generator-vue` (`@polyspec/generator-vue`) | SSR, 기준 HTML 7/7 parity, list ListV2 |
| 렌더러 | `generator-svelte` (`@polyspec/generator-svelte`) | SSR, 기준 HTML 7/7 parity, list ListV2 |
| 도구 | `cli` (`@polyspec/cli`, bin `polyspec`) | describe/check/explain/list-widgets |

## 빠른 링크

- [문서 색인](/README) — 전체 문서 목차
- [SPEC-V2 (헌법)](/SPEC-V2) — v2 단일 진실 · 역할 슬롯 · 조건맵 · 합성 · 금지키 게이트(§6) · list-spec(§9)
- [표현식 문법](/EXPRESSION-GRAMMAR) — 4언어 표현식 엔진 단일 진실 (토큰/AST/값 멱등, eval 금지)
- [polyspec CLI](/POLYSPEC-CLI) · [polyspec MCP](/POLYSPEC-MCP) — v2 작성 도구층
- 크로스-검증 콘솔 `examples/cross-check-console` — 4언어 검증 × 3프레임워크 SSR · list 탭
- [YAML 스펙 형식](/SPEC) — 스펙 작성법 (v1 호환)
- [검증 규칙](/VALIDATION-RULES) — 규칙 레퍼런스
- [조건식 파서](/CONDITION-PARSER) · [조건부 표시 (v1 호환)](/DISPLAY-CONDITIONS)
