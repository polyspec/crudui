...
> 선언적 폼 스펙. legacy(Legacy)를 발판 삼아 초월한다.

이 문서는 CRUDUI의 **헌법**이다. 표현식 엔진 상세는 [EXPRESSION-GRAMMAR.md](./EXPRESSION-GRAMMAR.md),
...
하게 렌더되며, legacy Legacy를 기능·구조 양면에서 초월하고 결국 대체한다 — 원칙으로
자기설명되어 설명 없이 읽힌다.

| 골 | 정의 | 달성 판정 |
|---|---|---|
| **G-A 단일 진실** | 폼의 모든 측면(구조·콘텐츠·검증·표시·외형·동작·다국어·타입옵션·합성)이 한 YAML에서 도출. 같은 개념 한 곳, 조건부 이중화 없음. | 코드에 숨은 폼 로직 0, 조건 전용 메타키 0 |
| **G-B 4언어 검증 멱등** | JS/PHP/Go/Rust가 같은 (스펙, 데이터) → 비트 동일한 결과(valid+errors). 표현식 토큰열/AST/평가값도 4언어 동일. | 공유 픽스처(현 1074 + CRUDUI 신규) 4언어 GREEN |
...
| **G-F legacy 초월·대체** | 기능 legacy ⊂ CRUDUI(+합성·다국어 메시지·동적 옵션·검증 확장점), 구조는 legacy보다 깨끗. 어댑터·번역기·오라클은 과도기 — 안정 후 legacy 폐기. CRUDUI를 번역 가능성으로 제약하지 않음. | 운영 스펙이 CRUDUI로 표현·검증, 안정 후 legacy 의존 0 |
| **G-G 게이트 영구 강제** | 멱등·parity는 측정·게이트, 미달이라고 기준을 낮추지 않음(정당한 약화만). CRUDUI 자체 게이트(영구) + legacy 오라클(과도기)이 정답 자동 생성. | CI가 멱등·parity·표현식 픽스처를 차단 |
...
- **R7 legacy 초월.** 어댑터·오라클은 과도기 도구다 — 결국 레거시를 폐기한다. **CRUDUI를
  번역 가능성으로 제약하지 않는다**: CRUDUI가 legacy을 초월하는 부분은 번역 불가가 정상이고
  그게 가치다. 번역기를 위해 CRUDUI를 희생하지 않는다. CRUDUI는 legacy 없이 자기 명세·게이트로 성립.
...
- **CRUDUI 자체 게이트 (영구 기준)**: EXPRESSION-GRAMMAR 명세 픽스처(토큰/AST/값) + 4언어
  멱등 + 3프레임워크 parity + CRUDUI 신규(값 반환·조건맵·경로비교) 직접 픽스처. legacy 없이 성립.
- **legacy 엔진 오라클 (과도기 보조)**: 운영 legacy 스펙을 legacy 엔진(기준)과 CRUDUI 번역→CRUDUI 엔진
  으로 교차검증(legacy 호환 범위). 번역기 왕복 무손실(`legacy→CRUDUI→legacy`=원본)로 번역 검증. legacy
  엔진이 멱등으로 신뢰되어 실스펙 정답을 자동 생성한다. **안정 후 오라클·번역기 폐기**.
- **게이트 코퍼스**: 자작 예시가 아니라 운영 스펙에서 추출한다.

## 7. 구현 상태 + 로드맵 (골 게이트 연결)

**현 crudui은 legacy legacy 포트다**(감사 실측). 1074/1074·7/7 GREEN은 legacy 의미론의
증거이지 CRUDUI 달성이 아니다 — parity 게이트는 legacy 행동을 핀해 G-A/G-F와 오히려
역방향이다. CRUDUI 구현 갭(각각 골 게이트로 강제):

- 역할 슬롯(`validate`/`design`/`behavior`/`options`) 미소비 — 스키마·검증기 4·
  generator 3 전부 legacy `rules`/외형키 모델 (G-A·G-B)
...
- 운영 스펙 코퍼스 wiring 0 — 판정 분모 미확보 (G-F)
...
→ 검증기 CRUDUI 슬롯 소비 → 표시로직 코드→스펙 이전 → legacy→CRUDUI 번역기 → 운영 스펙 반입 →
토큰/AST·CRUDUI신규 픽스처 → 영구 게이트.

**표현식 4언어 토큰/AST 멱등 숙제**: PHP는 CRUDUI AST 엔진으로 JS와 38케이스 일치(완료).
...
미확정. CRUDUI도 복잡 케이스를 마법으로 단순화하지 못한다 — 가치는 규칙 일관·매직 제거·
역할 분리다.
...
`crudui.schema.json` 에 `List`/`Column`/`CellFormat`/`Pagination`/`Sort`/`ListAction`
