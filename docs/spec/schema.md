...
| **G-F legacy 초월·대체** | 기능 legacy ⊂ CRUDUI(+합성·다국어 메시지·동적 옵션·검증 확장점), 구조는 legacy보다 깨끗. 어댑터·번역기·오라클은 과도기 — 안정 후 legacy 폐기. CRUDUI를 번역 가능성으로 제약하지 않음. | 실운영 577 스펙이 CRUDUI로 표현·검증, 안정 후 legacy 의존 0 |
...
- **legacy 엔진 오라클 (과도기 보조)**: 실운영 legacy 스펙을 legacy 엔진(기준)과 CRUDUI 번역→CRUDUI 엔진
