...
CRUDUI의 초기 패키지 버전은 `0.0.1`입니다. 현재 API 이름에는 구현 세대를
나타내는 접미사를 사용하지 않습니다. 패키지 루트는 현재 검증기와 폼 렌더러를
제공합니다. 구형 구현은 명시적인 `legacy` 모듈을 사용합니다. 공유 규칙과
표현식 유틸리티는 레거시 모듈에 의존하지 않습니다. 제거한 버전 경로에는
호환 별칭을 제공하지 않습니다.

기계가 읽는 계약은
[`schema/crudui.schema.json`](../../schema/crudui.schema.json)에 관리합니다.
폼과 목록 선언 형태를 검증합니다. `make docs-schema`는 규칙을 생성하거나
교체하지 않고 이 기준과 공유 고정 데이터를 검사합니다.
