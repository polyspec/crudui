...
삽입합니다. 노드 문법은 [폼 마크업](form-markup.ko.md)에서 정의합니다. `multiple: only`로 선언한 컬렉션에는 `add-row` 컨트롤이 없으며, 비어 있으면 헤더와 빈 본문을
렌더링합니다. 이 컬렉션의 데이터가 없으면 초기 행 없이 행이 0개입니다. `design.show`는 행 개수와 별도로
표시 여부를 결정합니다. 컬렉션을 숨겨도 데이터를 유지하고 규칙은 건너뛰며, 숨겨진 행의 이름 있는
컨트롤은 네이티브 제출에 포함합니다.
