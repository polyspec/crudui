...
- `multiple.header: sticky`는 `crudui-node--sticky`를 추가하고, 행 루트 스타일이
  ...
  정렬 위치, 푸터 높이를 뺀 값이므로 그 헤더가 고정선에 닿을 때 스크롤이 끝납니다. 스크롤
  컨테이너는 고정 헤더가 따르는 컨테이너로, 내용이 아직 넘치는지와 관계없이 세로 overflow가
  `auto` 또는 `scroll`인 가장 가까운 조상이고 없으면 문서입니다. 따라서 페이지, 프레임,
  스크롤 박스 안의 폼이 같게 동작합니다. `connectRows`는 이 세 길이(`--crudui-scroll-height`,
  `--crudui-form-end-extent`, `--crudui-form-end-top`)를 렌더링이 교체하지 않는 연결 요소에
  게시합니다. 이 한계는 스크롤 영역에서 폼
