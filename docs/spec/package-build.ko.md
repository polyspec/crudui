...
LTS 채널이 없는 런타임과 도구는 프로젝트가 지원하는 최신 안정 릴리스를
사용합니다. 명세가 명시적으로 요구하지 않는 프리릴리스 버전은 사용하지
않습니다.

저장소 실행 설정은 숫자로 된 런타임 또는 도구 릴리스 대신 현재 채널을
선택합니다. Node.js CI는 `lts/*`를 선택하고 Node.js 컨테이너 단계는 `lts` 이미지
태그를 사용하며 CI는 현재 안정 npm 릴리스를 설치합니다. 후보 빌드는 이 채널을
한 번 해석하고 결과 이미지와 런타임 버전을 검증 근거에 기록합니다. 이후 빌드는
더 새로운 LTS 또는 안정 릴리스를 적용하고 새 검증 근거를 생성합니다. 패키지 잠금
파일은 해석한 패키지 버전을 기록하며 런타임 릴리스를 선택하지 않습니다.
