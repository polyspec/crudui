<script lang="ts">
  ...
  /** Field spec key (property name). */
  export let name: string;
  /** Field specification. */
  export let spec: SpecNode;
  /** Full dot path of this field in the form data. */
  export let path: string = name;
  /** Parent group dot path ('' / undefined for top-level fields). */
  ...
  $: html = renderField(name, spec, path, parentPath, state);
</script>
