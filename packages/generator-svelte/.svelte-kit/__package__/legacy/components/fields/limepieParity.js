 ...
 */
export function phpFloatString(v) {
 ...
 */
export function applyDefaultString(value, def) {
 ...
 */
export function ruleNameForPath(path) {
    let suffix = '';
    let p = path;
    if (p.endsWith('[]')) {
        suffix = '[]';
 ...
 */
export function itemEntries(items) {
    if (items === null || items === undefined)
        return [];
    if (Array.isArray(items)) {
        if (items.length > 0 && items.every((e) => Array.isArray(e) && e.length === 2)) {
            return items.map((e) => [phpString(e[0]), e[1]]);
        }
 ...
 */
export function wrapperLayerName(path, keyPrefix) {
 ...
 */
export function limepieDataAttrs(spec, path) {
    return {
 ...
 */
export function nextChoiceToken() {
...
export function resetChoiceTokens() {
