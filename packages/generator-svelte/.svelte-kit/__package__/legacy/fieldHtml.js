 ...
 */
import { applyDefaultString, cleanStr, escAttr, escText, itemEntries, leafName, limepieDataAttrs, nextChoiceToken, phpFloatString, phpString, phpTruthy, ruleNameForPath, styleString, } from './components/fields/limepieParity';
...
const text = (ctx) => {
    ...
    const elementStyle = `${spec.element_style ?? ''}${ctx.readonly ? ' pointer-events: none;' : ''}`;
    const classes = ['valid-target', 'form-control'];
    if (spec.element_class)
        classes.push(spec.element_class);
    const input = `<input type="text" name="${escAttr(name)}" value="${escAttr(displayValue)}"` +
    ...
    const dynActive = typeof spec.dynamic_onchange === 'string' && spec.dynamic_onchange !== '' && Boolean(ctx.buttonsHtml);
...
const email = (ctx) => {
    ...
    if (spec.element_class)
        classes.push(spec.element_class);
    const input = `<input type="email" name="${escAttr(name)}" value="${escAttr(displayValue)}"` +
        ` class="${escAttr(classes.join(' '))}"` +
        (ctx.disabled ? ' disabled=""' : '') +
        (ctx.readonly ? ' readonly=""' : '') +
        (spec.placeholder ? ` placeholder="${escAttr(t(spec.placeholder))}"` : '') +
...
const password = (ctx) => {
    ...
    const input = `<input type="password" name="${escAttr(name)}" value="${escAttr(value)}"` +
...
const number = (ctx) => {
    ...
    if (spec.element_class)
        classes.push(spec.element_class);
    const input = `<input type="number" name="${escAttr(name)}" value="${escAttr(displayValue)}"` +
        ` class="${escAttr(classes.join(' '))}"` +
        (ctx.disabled ? ' disabled=""' : '') +
        (ctx.readonly ? ' readonly=""' : '') +
        (spec.placeholder ? ` placeholder="${escAttr(t(spec.placeholder))}"` : '') +
...
const textarea = (ctx) => {
    ...
    const maxLength = spec.counter && typeof rulesMaxlength === 'number' ? rulesMaxlength : undefined;
    const rows = spec.rows ?? 5;
    const elementStyle = styleString(spec.element_style);
    const classes = ['valid-target', 'form-control'];
    if (spec.element_class)
        classes.push(spec.element_class);
    const ta = `<textarea name="${escAttr(name)}" class="${escAttr(classes.join(' '))}"` +
        (ctx.disabled ? ' disabled=""' : '') +
        (ctx.readonly ? ' readonly=""' : '') +
        ` rows="${rows}"` +
...
const select = (ctx) => {
        ...
        for (const [key, val] of itemEntries(items)) {
            if (typeof val === 'object' && val !== null) {
                if (isMultiLangText(val)) {
                    ungrouped.push({ value: key, label: t(val) });
                }
                else {
                    for (const [subKey, subVal] of Object.entries(val)) {
                        ...
                        groups[key].push({ value: subKey, label: t(subVal) });
                    }
                }
            }
            else {
    ...
    for (const o of ungrouped)
        optionsHtml += opt(o.value, o.label);
    ...
    const inlineOnchange = typeof spec.onchange === 'string' && spec.onchange ? `${minifyJs(spec.onchange)};` : null;
    const dynActive = typeof spec.dynamic_onchange === 'string' && spec.dynamic_onchange !== '' && Boolean(ctx.buttonsHtml);
    if ((inlineOnchange && !readonlyTruthy) || dynActive) {
        const dataAttrs = limepieDataAttrs(spec, ctx.path);
        let elementStyle = spec.element_style ?? '';
        if (readonlyTruthy) {
            elementStyle +=
        ...
        const disables = Array.isArray(spec.disables)
            ? spec.disables.map((d) => phpString(d))
            : [];
        let rawOptionHtml = '';
        if (items && typeof items === 'object' && !('model' in items)) {
            for (const [itemValueRaw, itemTextRaw] of Object.entries(items)) {
                let itemTextVal = itemTextRaw;
                if (itemTextVal !== null && typeof itemTextVal === 'object' && !Array.isArray(itemTextVal)) {
                    const langText = itemTextVal[ctx.language];
                    if (langText !== undefined)
                        itemTextVal = langText;
                }
                ...
                if (itemTextVal !== null && typeof itemTextVal === 'object') {
                    ...
                    for (const [subValRaw, subTextRaw] of Object.entries(itemTextVal)) {
        ...
        const selectHtml = `<select class="valid-target form-select${spec.element_class ? ` ${String(spec.element_class)}` : ''}"` +
    ...
    const sel = `<select name="${escAttr(name)}" class="${escAttr(classes.join(' '))}"` +
...
const choice = (ctx) => {
    ...
    const defaultStr = spec.default !== undefined && spec.default !== null && !Array.isArray(spec.default)
    ...
    const containerClass = 'btn-group btn-group-toggle' +
    ...
    const readonlyMap = new Set();
    const ro = spec.readonly;
    if (ro && typeof ro === 'object' && !Array.isArray(ro)) {
        const arr = ro[effectiveValue];
        if (Array.isArray(arr))
            for (const k of arr)
                readonlyMap.add(String(k));
    }
    if (Array.isArray(spec.disableds)) {
        for (const k of spec.disableds)
            readonlyMap.add(String(k));
    }
    ...
    const inlineOnchange = typeof spec.onchange === 'string' && spec.onchange ? minifyJs(spec.onchange) : null;
    const inlineOnclick = typeof spec.onclick === 'string' && spec.onclick ? minifyJs(spec.onclick) : null;
...
const date = (ctx) => {
    ...
    const input = `<input type="date" name="${escAttr(name)}" value="${escAttr(formattedValue)}"` +
        ` class="valid-target form-control"` +
        (ctx.disabled ? ' disabled=""' : '') +
        (ctx.readonly ? ' readonly=""' : '') +
        attrs(limepieDataAttrs(spec, ctx.path)) +
...
export function datetimeNeedsRawHtml(spec) {
    const event = spec.event;
    if (event && Array.isArray(event.type) && event.type.length > 0)
        return true;
    const onchange = spec.onchange;
    return typeof onchange === 'string' && onchange !== '';
}
function inlineJs(fn) {
 ...
 */
export function datetimeLegacyRawHtml(ctx) {
    const { spec } = ctx;
    const name = bracket(ctx);
    const rawValue = phpString(ctx.value) || phpString(spec.default);
    ...
    if (event) {
        ...
        for (const ev of types) {
            if (ev === 'onchange')
                onchange += ` onchange="${escAttr(inlineJs(fn))}"`;
            if (ev === 'onload')
                onchange += ` data-onload="${escAttr(inlineJs(fn))}"`;
        }
    }
    ...
    const style = typeof spec.style === 'string' && spec.style ? ` style="${escAttr(spec.style)}"` : '';
    return (`<input type="datetime-local" class="valid-target form-control"` +
...
const datetime = (ctx) => {
    ...
    return (`<input type="datetime-local" name="${escAttr(name)}" value="${escAttr(formattedValue)}"` +
        ` class="valid-target form-control"` +
        (ctx.disabled ? ' disabled=""' : '') +
        (ctx.readonly ? ' readonly=""' : '') +
...
const dummy = (ctx) => {
    ...
    const items = spec.items;
    if (items && typeof items === 'object' && !Array.isArray(items) && !('model' in items)) {
        const looked = items[phpString(v)];
        if (looked !== undefined)
            v = looked;
    }
    ...
    return (`<div${cls ? ` class="${escAttr(cls)}"` : ' class=""'}` +
        (style ? ` style="${escAttr(style)}"` : '') +
        `>${html}</div>`);
};
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
const dummyInput = (ctx) => {
    const { spec, t } = ctx;
    const name = bracket(ctx);
    const displayValue = applyDefaultString(ctx.value, spec.default);
    const elementStyle = `${spec.element_style ?? ''}${spec.readonly ? ' pointer-events: none;' : ''}`;
...
const image = (ctx) => {
    ...
    if (spec.prepend) {
        ...
        if (spec.prepend !== null && typeof spec.prepend === 'object' && !Array.isArray(spec.prepend)) {
            prependText = phpString(spec.prepend[ctx.language]);
        }
        else {
    ...
    const sizeAttrs = ` data-max-width="${escAttr(maxWidth)}" data-min-width="${escAttr(minWidth)}"` +
    ...
    const validAttrs = ` data-name="${escAttr(leafName(ctx.path))}" data-rule-name="${escAttr(ruleNameForPath(ctx.path))}"`;
    const html = prependHtml +
...
// ---------------------------------------------------------------------------
function itemText(v, language) {
    if (v !== null && typeof v === 'object' && !Array.isArray(v)) {
        const localized = v[language];
...
const search = (ctx) => {
    ...
    const disables = Array.isArray(spec.disables) ? spec.disables.map(String) : [];
    let prependHtml = '';
    if (spec.prepend) {
        const prependClass = spec.prepend_class ? ` ${String(spec.prepend_class)}` : '';
    ...
    let appendHtml = '';
    if (spec.append)
        appendHtml = `<span class="input-group-text">${String(spec.append)}</span>`;
    let containerClass = '';
    if (!prependHtml)
        containerClass += ' input-group-first';
    if (!appendHtml && !spec.multiple)
    ...
    if (spec.items !== undefined && spec.items !== null) {
        for (const [itemKey, itemValue] of itemEntries(spec.items)) {
            ...
            let appendText = '';
            let optionClass = '';
            if (itemValue !== null && typeof itemValue === 'object' && 'id' in itemValue) {
                const o = itemValue;
    ...
    const selectHtml = `<select class="valid-target form-control${escAttr(elementClass)}"${styleAttr}` +
    ...
    const callback = typeof spec.callback === 'string' && spec.callback
...
const tinymce = (ctx) => {
    ...
    const upload = spec.fileserver ?? 'upload';
    const readonlyJs = spec.readonly !== undefined ? (spec.readonly ? 'true' : 'false') : 'false';
    const displayValue = applyDefaultString(ctx.value, spec.default);
    const editorId = `tinymce${id}`;
    const ta = `<textarea id="${escAttr(editorId)}" class="valid-target form-control tinymcearea"` +
...
const button = (ctx) => {
    ...
    const displayValue = applyDefaultString(ctx.value, spec.default);
    const textVal = spec.text !== undefined ? t(spec.text) : '';
    ...
    const hidden = `<input type="hidden" class="valid-target form-control" readonly=""` +
        ` name="${escAttr(name)}" data-name="${escAttr(dataAttrs['data-name'])}"` +
        ` data-rule-name="${escAttr(ruleName)}" value="${escAttr(displayValue)}"` +
        ` data-default="${escAttr(dataAttrs['data-default'])}"/>`;
    const btn = `<input type="button" class="btn${spec.button_class ? ` ${String(spec.button_class)}` : ''}"` +
...
const switcher = (ctx) => {
    const { spec, t } = ctx;
    const name = bracket(ctx);
    const onLabel = spec.on_label ?? t('on') ?? 'On';
    const dataAttrs = limepieDataAttrs(spec, ctx.path);
    ...
    const input = `<input type="checkbox" name="${escAttr(name)}" value="1" class="valid-target"` +
        ` data-rule-name="${escAttr(dataAttrs['data-rule-name'])}"` +
...
export function checkboxInnerHtml(ctx) {
    const { spec, t } = ctx;
    ...
    const style = styleString(spec.style);
    const input = `<input type="checkbox" name="${escAttr(fullBracketName)}" value="1"` +
        (checked ? ' checked=""' : '') +
        (ctx.disabled || ctx.readonly ? ' disabled=""' : '') +
        ` class="valid-target"` +
...
export function getFieldHtml(type) {
