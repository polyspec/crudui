 ...
 */
function processSingleLang(value, language) {
    ...
    const appendLangProperties = { ...value };
    for (const k of LANG_CHILD_UNSET)
        delete appendLangProperties[k];
    if (isLangAppend && value.langs === undefined && isPlainObject(appendLangProperties.rules)) {
        const rules = { ...appendLangProperties.rules };
        delete rules.required;
        appendLangProperties.rules = rules;
    }
    if (isRemoveLabel)
        delete appendLangProperties.label;
    ...
    if (Array.isArray(langsSpec)) {
        ...
        const ordered = Object.keys(langsSpec).sort((a, b) => DESIRED_ORDER.indexOf(a) - DESIRED_ORDER.indexOf(b));
        const filtered = {};
    ...
    // buildLanguageGroup.
    const languagePackName = typeof value.lang_name === 'string' && value.lang_name !== ''
        ? value.lang_name
        : '언어팩'; // \Limepie\__('core', '언어팩') — ko locale (see header)
    const langClass = value.class !== undefined ? ` ${String(value.class)}` : '';
    const langGroupClass = value.lang_group_class !== undefined ? ` ${String(value.lang_group_class)}` : '';
...
function isSingleLangSpec(key, value) {
    return (isPlainObject(value) && typeof value.lang === 'string' && !/\[\]$/.test(key));
}
/**
 ...
 */
function patchDisplaySwitchLists(properties) {
    for (const fields of Object.values(properties)) {
        if (!isPlainObject(fields) || !isPlainObject(fields.display_switch))
            continue;
        ...
        for (const [scriptKey, elements] of Object.entries(ds)) {
            ...
            for (const raw of elements) {
                ...
                if (typeof element === 'string' &&
                    ...
                    properties[element].lang === 'append') {
                    const langElement = `${element}_langs`;
                    if (!next.includes(langElement))
                        next.push(langElement);
                }
            }
            ds[scriptKey] = next;
            changed = true;
 ...
 */
function walkNode(node, language) {
 ...
 */
export function applyLangAppendTransform(spec, language = 'ko') {
