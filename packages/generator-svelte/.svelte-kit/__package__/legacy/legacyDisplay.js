 ...
 */
import { phpString, styleString } from './components/fields/limepieParity';
/** \Limepie\genRandomString() charset: no i/l/o/0/1. */
const TOKEN_CHARS = 'abcdefghjkmnpqrstuvwxyz23456789';
/** 32-bit FNV-1a string hash — deterministic display-token seed. */
function fnv1a(seed) {
    let h = 0x811c9dc5;
...
export function displayTokenForSeed(seed) {
    let h = fnv1a(seed);
    let token = '';
    for (let i = 0; i < 5; i++) {
...
export function minifyJs(input) {
    if (input.trim() === '')
        return input;
    let out = input;
    out = out.replace(/\s*("(?:[^"\\]|\\[\s\S])*"|'(?:[^'\\]|\\[\s\S])*')\s*|\s*\/\*(?!!|@cc_on)[\s\S]*?\*\/\s*|\s*(?<![:=])\/\/.*(?=[\n\r]|$)|^\s+|\s+$/g, '$1');
...
export function isDisplaySwitchMap(v) {
    return isPlainObject(v);
}
function elementClassFor(element, token) {
    return element.split('[]').join('__') + `_${token}`;
}
function switchBranches(ds) {
    const out = [];
    for (const [scriptKey, elements] of Object.entries(ds)) {
        if (!Array.isArray(elements))
            continue;
        out.push([scriptKey, elements.map((e) => String(e).trim()).filter((e) => e !== '')]);
    }
    return out;
}
function buildOnChangeCode(branches, token, allElementsSelector) {
    const jsConditions = [];
    for (const [scriptKey, elements] of branches) {
        if (elements.length === 0)
            continue;
        const shows = elements.map((el) => `$self.closest('.form-group').find('.${elementClassFor(el, token)}').show();`);
        const condition = `((this.type === 'checkbox' || this.type === 'radio')\n` +
...
function setupVisibilityAt(level, key, token) {
    ...
    for (const [scriptKey, elements] of branches) {
        for (const el of elements) {
            ...
            const existingClasses = String(sibling.class ?? '').split(' ');
            ...
            if (!isPlainObject(sibling.display_target_condition_style)) {
                ...
                for (const dk of diffKeys)
                    init[dk] = 'display: none;';
                sibling.display_target_condition_style = init;
            }
            const styleMap = sibling.display_target_condition_style;
            ...
            const existingStyles = String(sibling.style ?? '').split(';');
            const defStr = defaultValue !== null && defaultValue !== undefined ? phpString(defaultValue) : null;
    ...
    if (code !== null)
        fields.onchange = code;
}
/**
 * Recursive pass over every `properties` level. Returns a DEEP CLONE.
 */
export function applyDisplaySwitchTransform(spec) {
    const clone = JSON.parse(JSON.stringify(spec));
    const walkLevel = (level, parentPath) => {
        for (const [childKey, value] of Object.entries(level)) {
            if (isPlainObject(value) && isPlainObject(value.properties)) {
                ...
                walkLevel(value.properties, childPath);
            }
        }
...
export function resolveRelativePath(targetPath, currentPath) {
    ...
    if (kept.filter((p) => p !== '').length === 0)
        return cleanPath;
    return kept.join('.') + '.' + cleanPath;
}
function getValueByDot(data, key) {
    let value = data;
...
function getDefaultByDot(rootSpec, key) {
    let property = rootSpec;
    for (const id of key.split('.')) {
        if (ROW_KEY_SEGMENT.test(id) || id === '*')
            continue;
        const props = property.properties;
        if (!isPlainObject(props))
            return null;
        const next = (props[id] ?? props[id + '[]']);
 ...
 */
export function resolveDisplayTargetParts(fieldSpec, parentDotPath, data, rootSpec) {
    const none = { addClass: [], style: null };
    ...
    let targetValue = getValueByDot(data, target);
    ...
    const result = { addClass: [], style: null };
    ...
    if (isPlainObject(classMap) && classMap[lookupKey] !== undefined) {
        ...
        if (cls !== '')
            result.addClass.push(cls);
    }
    return result;
}
/** True when the spec carries legacy condition maps. */
export function hasDisplayTargetConditionMaps(fieldSpec) {
    return (fieldSpec.display_target_condition_style !== undefined ||
 ...
 */
export function legacyWrapperClassName(spec, allOfClassName, condition) {
    const classes = ['form-element-wrapper'];
    if (spec.class)
        classes.push(String(spec.class).trim());
    if (allOfClassName)
        classes.push(allOfClassName);
    if (condition)
        classes.push(...condition.addClass);
 ...
 */
export function legacyWrapperStyle(spec, allOfStyle, condition, hidden = false) {
    const parts = [];
    ...
    if (allOfStyle)
        parts.push(allOfStyle);
    if (condition?.style)
        parts.push(condition.style);
    if (hidden)
        parts.push('display: none');
    if (parts.length === 0)
        return undefined;
    return styleString(parts.join('; '));
}
