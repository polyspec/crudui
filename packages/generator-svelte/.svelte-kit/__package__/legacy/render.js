 ...
 */
import { escAttr, escText, styleString, wrapperLayerName, } from './components/fields/limepieParity';
import { generateUniqid, getValueByPath } from './utils';
import { resolveDisplayTargetParts, legacyWrapperClassName, legacyWrapperStyle, } from './legacyDisplay';
import { checkboxInnerHtml, datetimeLegacyRawHtml, datetimeNeedsRawHtml, getFieldHtml, switcherHtml, } from './fieldHtml';
// ---------------------------------------------------------------------------
// shared presentation helpers
// ---------------------------------------------------------------------------
/** Legacy multiple detection (Group::determineIsArray). */
function isMultipleSpec(spec) {
...
function inputGroupWrapperClassName(spec, rowIndex = 0) {
    const classes = ['input-group-wrapper'];
    if (rowIndex > 0)
        classes.push('clone-element');
    const wc = spec.wrapper_class;
    if (typeof wc === 'string' && wc)
        classes.push(wc);
    return classes.join(' ');
}
/** form-element-wrapper class/style chain (no all_of in the fixtures). */
function wrapperPresentation(spec, parentDotPath, state) {
    const conditionParts = resolveDisplayTargetParts(spec, parentDotPath, state.data, state.rootSpec);
    return {
...
function openWrapper(className, style, name) {
    return (`<div class="${escAttr(className)}"` +
...
// ---------------------------------------------------------------------------
function addCSlashesQuote(s) {
    return s.split('"').join('\\"');
}
function hasDynamicOnchange(spec) {
    const d = spec.dynamic_onchange;
    return typeof d === 'string' && d !== '';
}
/** Raw legacy row-buttons HTML (matches FormGroup.legacyRowButtonsHtml). */
function legacyRowButtonsHtml(spec) {
    const dynRaw = spec.dynamic_onchange;
    const dyn = typeof dynRaw === 'string' && dynRaw !== '' ? ` onclick="${addCSlashesQuote(dynRaw)}"` : '';
...
function reactRowButtonsHtml(spec) {
    const sortable = spec.sortable === true;
    const multipleMax = spec.multiple_max;
    const maxAttr = multipleMax !== undefined && multipleMax !== null
...
// FormField
// ---------------------------------------------------------------------------
function stripArraySuffix(path, spec) {
...
export function renderField(name, spec, rawPath, parentPath, state) {
    ...
    const label = spec.label ? state.t(spec.label) : undefined;
    const ctx = {
        spec,
        value,
        path,
    ...
    if (fieldType === 'checkbox' || fieldType === 'switcher') {
        ...
        return (openWrapper(presentation.className, presentation.style, wrapperName) +
            `<div class="checkbox">` +
            `<h6>` +
    ...
    const labelTag = label && fieldType !== 'hidden'
        ? `<h6${spec.label_class ? ` class="${escAttr(spec.label_class)}"` : ' class=""'}>${escText(label)}</h6>`
        : '';
...
// multiple leaf field
// ---------------------------------------------------------------------------
function multipleRowKeys(value) {
    if (Array.isArray(value) && value.length > 0)
        return value.map(() => generateUniqid());
...
function renderMultipleLeaf(name, spec, path, parentPath, state) {
    const fieldType = String(spec.type ?? '');
    const value = getValueByPath(state.data, path);
    const wrapperName = wrapperLayerName(path, state.keyPrefix || undefined);
    const presentation = wrapperPresentation(spec, parentPath ?? '', state);
...
export function renderGroup(name, spec, path, parentPath, state) {
    if (isMultipleSpec(spec)) {
...
function multipleGroupRowKeys(value) {
    if (Array.isArray(value) && value.length > 0)
        return value.map(() => generateUniqid());
    if (value !== null && typeof value === 'object' && !Array.isArray(value)) {
        const keys = Object.keys(value);
...
function renderMultipleGroup(name, spec, path, parentPath, state) {
    const wrapperName = wrapperLayerName(path, state.keyPrefix || undefined);
    const presentation = wrapperPresentation(spec, parentPath ?? '', state);
    const label = spec.label ? state.t(spec.label) : undefined;
    const value = getValueByPath(state.data, path);
    ...
    let rowsHtml = '';
    rowKeys.forEach((rowKey, rowIndex) => {
        const rowValue = value !== null && typeof value === 'object' && !Array.isArray(value)
            ? value[rowKey]
            : undefined;
        let fieldsHtml = '';
        if (props) {
            for (const [fieldName, fieldSpec] of Object.entries(props)) {
                fieldsHtml += renderField(fieldName, fieldSpec, `${path}.${rowKey}.${fieldName.split('[]').join('')}`, `${path}.${rowKey}`, state);
            }
        }
        let buttonsBlock = '';
        if (showButtons) {
...
// ---------------------------------------------------------------------------
function renderFooter(spec, state) {
    ...
    const renderLegacyButtons = (list) => {
        let html = '';
        for (const b of list) {
            const type = b.type ?? '';
            const txt = b.text !== undefined ? state.t(b.text) : '';
            const cls = b.class ?? '';
    ...
    const submitText = typeof spec.submit_button_text === 'string' && spec.submit_button_text
        ? spec.submit_button_text
        : '저장';
    const listText = typeof spec.list_button_text === 'string' && spec.list_button_text
    ...
    if (hasAddButtons)
        inner += `<span>${renderLegacyButtons(addButtons)}</span>`;
    if (showListButton) {
 ...
 */
export function renderFormContent(spec, state) {
    let html = '';
    const label = spec.label ? state.t(spec.label) : '';
    const description = spec.description ? state.t(spec.description) : '';
    if (label)
        html += `<label class="form-label">${escText(label)}</label>`;
    if (description)
        html += `<div class="form-description">${escText(description)}</div>`;
