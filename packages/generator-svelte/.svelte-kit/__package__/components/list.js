...
export function nodeClass(base, node) {
...
export function headerClass(col) {
...
export function sortMarker(col) {
    return col.sortable ? '↕' : '';
}
/**
 ...
 */
export function sortDir(vm, col) {
    const s = vm.sort;
    if (!s)
        return undefined;
    if (s.field === col.field || s.field === col.key)
        return s.dir;
    return undefined;
}
function escAttr(s) {
 ...
 */
export function actionBehaviorAttrs(action) {
    if (!action.behavior)
        return '';
    let out = '';
...
export function actionHtml(action) {
    ...
    if (action.format?.type === 'link') {
        ...
        const target = typeof action.format.options.target === 'string'
