...
export function isUnsupported(w) {
    return w.unsupported === true;
}
/** Raw prepend/append affix span html. */
function affixHtml(affix) {
    if (!affix)
        return '';
...
function rawControl(w, selectedAttr = 'empty') {
    if (w.tag === 'select') {
...
function groupButtonHtml(o, type, shared, labelClass) {
    const attrs = {
        ...shared,
        type,
        value: o.value,
    ...
    if (type === 'radio')
        attrs['data-is-default'] = o.isDefault ? '1' : '';
...
export function inputGroupBody(w) {
    return affixHtml(w.prepend) + rawControl(w) + affixHtml(w.append);
}
/** btn-group full html: `<div {attrs}>` + per-item input/label pairs. */
export function btnGroupHtml(w) {
    const type = w.kind === 'choice' ? 'radio' : 'checkbox';
    const shared = (w.extra?.input ?? {});
...
export function fileGroupBody(w) {
    const display = w.extra?.display;
    const fileAttrs = w.extra?.file ?? {};
    return (affixHtml(w.prepend) +
 ...
 */
export function searchHtml(w) {
    const fieldSearch = `<div class="input-group field-search">` +
        affixHtml(w.prepend) +
 ...
 */
export function widgetRootRaw(w) {
    if (isUnsupported(w))
        return null;
    if (w.layout === 'bare')
        return rawControl(w);
    ...
    if (w.layout === 'button') {
        ...
        return (`<script nonce="">${w.script ?? ''}<\/script>` +
            rawVoid('input', hidden) +
            rawVoid('input', w.attrs));
    }
    if (w.layout === 'btn-group')
        return btnGroupHtml(w);
    if (w.layout === 'search')
        return searchHtml(w);
    return null;
}
...
export function rowButtonsHtml(s) {
    ...
    if (s.copy)
        html += `<button type="button" class="btn btn-copy"> </button>`;
