/**
 * CRUDUI type-specific inner emitters (Svelte) — the `.input-group` inner markup per
 * field type. Structure (envelope, input-group, data-attr trio) is the verified
 * Legacy envelope (generator-svelte/src/fieldHtml.ts REGISTRY); APPEARANCE is
 ...
 * LocalizedText. NO legacy meta key (element_class/button_class/prepend_class/style/
 ...
 * (data-name/data-rule-name/data-default) is the legacy validation contract.
 * These emitters are byte-identical to the React CRUDUI reference; the shared parity
 * gate (tests/fixtures/form-render) pins them.
 *
...
/** The legacy data-attr trio for the main node. */
