/**
 * v2 design-slot resolver (Svelte) — appearance = visibility (`show`) + per-
 * DOM-node appearance map (R8). SPEC-V2 §3 common-role distribution; mirrors
 ...
 * into this one slot's node map. No v1 meta key is read here — the renderer
 * derives EVERY appearance value from `design` alone.
 *
 * Slot polymorphism (Slot<DesignSlot>):
