/**
 * v2 render errors (Svelte) — surfaced failures, never silent.
 *
 ...
 * Byte-identical to the React v2 reference (shared 3-framework parity gate).
 */
...
export class UnsupportedFieldTypeError extends Error {
  ...
  constructor(type: string, path: string) {
    super(`Unsupported field type "${type}" at "${path}" (no v2 emitter registered)`);
