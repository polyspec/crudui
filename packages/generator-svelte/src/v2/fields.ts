 ...
 * node), and CONTENT (placeholder/prepend/append/content) from `t()` over
 * LocalizedText. NO v1 meta key (element_class/button_class/prepend_class/style/
 * onchange) is read here.
 *
 ...
 *                  no expr eval, no v1 event→onchange transform).
 ...
 * `{ script }`. NO minify, NO v1 event→onchange transform.
 */
...
/**
 * A DYNAMIC items source is a `{ model, ... }` descriptor (DB-backed). The v1
...
const choice: Emitter = (ctx) => {
  ...
  // design.main.class fills the per-item LABEL slot (v1 element_class was the label).
...
const dummy: Emitter = (ctx) => {
  ...
  // RAW html passthrough — value goes through nl2br only (not escaped), v1 parity.
...
const imageViewer: Emitter = (ctx) => {
  ...
  if (Array.isArray(v) && v.length > 0) {
    ...
    // Hardcoded Korean empty literal — kept verbatim (v1 does not translate it).
    body = '이미지가 없습니다.';
  }
