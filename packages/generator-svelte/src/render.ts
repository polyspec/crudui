/**
 * CRUDUI SSR renderer (Svelte) — field → HTML.
 *
 * Flow (the four mandated stages):
 *   (1) CRUDUI spec → (2) CRUDUI compose (expand $ref/$patch into a single spec) →
 *   (3) resolve `design` slots + condition maps via the shared expr engine →
 *   (4) emit the SSR HTML envelope.
 *
 * The envelope is the VERIFIED Legacy/Svelte-reference structure
 * (generator-svelte/src/render.ts), reproduced byte-compatibly after
 * normalization. What changes from legacy is the SOURCE of every appearance/
 * visibility decision: legacy read scattered meta keys (class/style/group_class/
 * wrapper_class/display_switch/display_target/legacyLang); CRUDUI reads ONLY the
 * `design` slot (per-node appearance + `show`) and the `multiple`/`lang`/`group`
 * buckets. No legacy meta key and no `eval` are used.
 *
 * This module is a string-building renderer (the same shape as the React/Vue CRUDUI
 * references); the Svelte 5 SSR component (Form.svelte) emits the produced HTML
 * through `{@html}`, so the four stages run under `svelte/server` render(). The
 * SSR output, after the shared normalizer, is identical to React/Vue (3-framework
 * parity, tests/fixtures/form-render).
 *
 ...
 * (the legacy legacy non-removal contract, legacyDisplay.ts:351). `show` absent =
 * always shown.
 */
