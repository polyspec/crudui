 ...
 * packages/generator-react/src/hooks/legacyDisplay.ts (the verified parity
...
import { phpString, styleString, type SpecNode } from './components/fields/limepieParity';
...
function buildOnChangeCode(
  ...
  for (const [scriptKey, elements] of branches) {
    ...
    const shows = elements.map(
      (el) => `$self.closest('.form-group').find('.${elementClassFor(el, token)}').show();`
    );
    const condition =
...
function setupVisibilityAt(level: Record<string, SpecNode>, key: string, token: string): void {
