/**
 * CRUDUI generator low-level helpers (Svelte) — framework-independent string/name/
 * path primitives. Reproduce the verified Legacy envelope conventions
 * (generator-svelte/src/components/fields/legacyParity.ts) so the CRUDUI envelope is
 * bit-identical to the proven legacy output after normalization. NO legacy meta key is
 * read here; these are pure structural primitives (escaping, name/id derivation,
 * php-string casts, deterministic uniqid, value lookup).
 *
 * The shared parity gate (tests/fixtures/form-render) pins these against the React
 * CRUDUI reference output; do not diverge.
 */
...
/** Deterministic Legacy-shaped uniqid: `__[13 hex]__`. */
