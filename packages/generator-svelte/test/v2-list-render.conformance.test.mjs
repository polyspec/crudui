/**
 * v2 list-render conformance — Svelte SSR vs the shared 3-framework parity gate.
 *
 * The read sister of v2-render.conformance.test.mjs. The shared fixture
 * tests/fixtures/v2-list-render/cases.json declares one `expected_html` per list
 * scenario (the React v2 LIST reference generator's OWN normalized output, SPEC
 * §9). This test runs the SVELTE list generator (renderListV2 → ListV2.svelte →
...
import { renderListV2, ComposeLoadError } from '../src/v2/index.ts';
import { normalizeHtml } from '../../../tests/fixtures/v2-render/normalize.mjs';

const HERE = path.dirname(fileURLToPath(import.meta.url));
const FIXTURE = path.resolve(HERE, '../../../tests/fixtures/v2-list-render/cases.json');
...
function render(c) {
  return renderListV2(c.spec, c.rows ?? [], svelteOptions(c.options));
}

const ERROR_CLASS_BY_CODE = { REF_FILE_NOT_FOUND: ComposeLoadError };

describe('v2 list render — Svelte reproduces the normalized expected_html', () => {
