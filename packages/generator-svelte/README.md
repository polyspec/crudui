...
legacy Legacy `Generator::write()` output (the host page owns the surrounding
page chrome).
...
`renderGroup`), and the parity helpers (`legacyParity`, the
...
- `src/components/fields/legacyParity.ts` — PHP-cast helpers (copied/adapted
