/**
 * Forward translator: v1 spec object → v2 spec object (SPEC-V2 §6 absorbs_legacy).
 *
 ...
 *
 * NEVER emits a forbidden meta key — the output passes the v2 meta-schema and the
 * recursive forbidden-scan. NEVER runs `eval`. NEVER mutates the v1 input.
 *
...
/** v1 design appearance keys → [v2 design node, sub-key]. `null` node = main node. */
...
/** v1 lang-* keys → [v2 lang sub-key, inverted?]. */
...
/** v1 multiple-* keys → v2 multiple sub-key. */
...
/** v1 behavior keys → v2 behavior sub-key. */
...
/** v1 composition directives absorbed into $patch. */
...
/**
 * Translate a v1 ROOT spec (a group with `properties`, or a bare properties map
 * entry) into v2. Returns the v2 spec plus the irreversibility log.
 */
export function translateV1ToV2(v1: V1Spec): TranslateResult {
  const notes: TranslateNote[] = [];
  const v2 = translateSpec(v1, [], notes);
  return { v2, notes };
}

/**
 * Translate ONE v1 field spec → v2. Recurses into `properties` (which runs the
 * sibling-aware display pass). `path` is the dotted trace for notes.
 */
function translateSpec(v1: V1Spec, path: string[], notes: TranslateNote[]): V2Spec {
  const out: V2Spec = {};
  ...
  for (const key of Object.keys(v1)) {
    ...
    if (key === 'properties' && isObject(value)) {
      out.properties = translateProperties(value as V1Spec, [...path, 'properties'], notes);
      continue;
    }
    // Bug: empty `properties:` (null / not a map) — a property-less group, NOT a
    // typeless options node. v1 `properties:` with no children parses to null and
    ...
    if (key === '$patch' && isObject(value)) {
      // Bug 4: a $patch payload may carry nested v1 field specs whose legacy
    ...
    if (key === 'rules' && isObject(value)) {
      ...
      for (const rk of Object.keys(r)) {
        if (rk === 'messages') {
          note(notes, path, 'messages', 'MESSAGES_NO_SLOT', 'per-rule messages have no v2 validate channel (gap, out of scope)');
          continue;
        }
        validate[rk] = r[rk];
      }
    ...
    if (key === 'messages') {
      note(notes, path, 'messages', 'MESSAGES_NO_SLOT', 'per-rule/per-language messages have no v2 slot (gap, out of scope)');
      continue;
    }
    ...
    if (key === 'display_target_condition_style') {
      const targetPath = String((v1.display_target as unknown) ?? '');
    ...
    if (BEHAVIOR_KEYS.has(key)) {
      // Bug 5: a boolean flag (onload:true) is not a v2 BehaviorAction
      ...
      if (typeof value === 'boolean') {
        note(notes, path, key, 'BEHAVIOR_FLAG_NORMALIZE', `boolean behavior flag "${key}:${value}" dropped (v2 BehaviorAction is string|{label,script}, not a flag)`);
        continue;
      }
      behavior[key] = value;
      continue;
    ...
    // -- lang dimension --
    if (key === 'lang') {
      // v1 `lang: append` (mode) or `lang: true`.
      ...
      if (sub === 'only' && isObject(value)) {
        ...
        for (const [langCode, langSpec] of Object.entries(value)) {
          overrides[langCode] = isObject(langSpec)
            ? translateSpec(langSpec as V1Spec, [...path, 'lang', 'only', langCode], notes)
            : langSpec;
        }
        lang[sub] = overrides;
      } else {
  ...
  // absent in v1, so the node falls outside the bit-identity round-trip gate.
  if (out.type === undefined && isObject(out.properties)) {
    out.type = 'group';
    note(notes, path, 'type', 'TYPE_GROUP_INJECTED', 'properties present without type — type:group injected (Field.required=[type]); adds a key absent in v1');
  }
...
/**
 * Translate a v1 `properties` MAP. Runs the sibling-aware display_switch pass
 ...
 * carried/absorbed.
 */
function translateProperties(props: V1Spec, path: string[], notes: TranslateNote[]): V2Spec {
  ...
  for (const fieldName of Object.keys(props)) {
    ...
    if (fieldName === '$patch' && isObject(child)) {
      // Bug 4: recursively translate nested v1 field specs inside the patch.
      const patch: Record<string, unknown> = {};
    ...
    const v2child = translateSpec(child as V1Spec, [...path, fieldName], notes);
    ...
    if (
      v2child.type === undefined &&
      v2child.$ref === undefined &&
      v2child.$patch === undefined &&
      v2child.properties === undefined &&
      Object.keys(v2child).length > 0
    ) {
      v2child.type = 'group';
      note(notes, [...path, fieldName], 'type', 'TYPE_GROUP_INJECTED', 'field-position node without type — type:group injected (Field.required=[type]); adds a key absent in v1');
    }

    // Inject any distributed design.show for this target field.
    const inject = distributed[fieldName];
    if (inject) {
      const design = isObject(v2child.design) ? (v2child.design as Record<string, unknown>) : {};
      design.show = inject;
      v2child.design = design;
    }
    out[fieldName] = v2child;
  }
  return out;
}
...
/**
 * v1 `display_target_condition_style` ({0:'display:none',1:'display:block'}) →
 * v2 `design.show` boolean condition map. Normalizes the style channel to a
...
/**
 * v1 `display_target_condition_class` ({val:'cls'}) → v2 `design.class`
...
function absorbPatchDirective(
  ...
  // Bug 4: a directive payload may carry nested v1 field specs whose legacy keys
...
/**
 * Translate ONE $patch payload value (Bug 4). A patch value that is a v1
...
function translatePatchValue(value: unknown, path: string[], notes: TranslateNote[]): unknown {
  ...
  if (looksLikeV1FieldSpec(value)) {
    return translateSpec(value as V1Spec, path, notes);
  }
...
/**
 * Whether an object is a v1 field spec (vs. a plain data/content map). True when
 ...
 */
function looksLikeV1FieldSpec(obj: Record<string, unknown>): boolean {
...
function attachSlot(out: V2Spec, name: string, slot: Record<string, unknown>): void {
...
function attachLang(out: V2Spec, lang: Record<string, unknown>): void {
...
function attachMultiple(out: V2Spec, multiple: Record<string, unknown>, flag: boolean | undefined): void {
...
function note(
  ...
  notes.push({ path: path.join('.'), v1Key, reason, detail });
}
