/**
 * v2 list-spec validation conformance — JS reference verification (SPEC-V2 §9).
 *
 ...
 * SPEC §9).
 *
 * It runs the SHARED fixture tests/fixtures/v2-list-validity/cases.json — the
...
import { validateListV2, ComposeLoadError } from './index';

const __filename = fileURLToPath(import.meta.url);
const __dirname = path.dirname(__filename);
// src/v2/validate-list -> repo root is five levels up.
const FIXTURE = path.resolve(
  __dirname,
  '../../../../../tests/fixtures/v2-list-validity/cases.json'
);
...
function run(c: ListValidityCase) {
  return validateListV2(c.spec, c.files ? { files: c.files } : {});
}

describe('v2 list validate — every case declares an engine expectation', () => {
...
describe('v2 list validate — engine:pass loads clean (no rows validated)', () => {
  for (const c of cases.filter((x) => x.engine === 'pass')) {
    test(c.name, () => {
      ...
      let result: ReturnType<typeof validateListV2> | undefined;
      expect(() => {
        result = run(c);
...
describe('v2 list validate — a forbidden meta key in the list tree is a LOAD ERROR', () => {
...
describe('v2 list validate — does NOT touch form-spec validateV2 (R7 parallel)', () => {
  test('validateListV2 takes no `data` argument and validates no rows', () => {
