 ...
 * Form Validator
 *
 * Core validation class for polyspec validation
 */
...
/**
 * Validator class for polyspec validation
 */
export class Validator {
  ...
  private evaluateCondition(expression: string, context: PathContext): boolean {
    try {
      ...
      if (this.options.debug) {
        console.warn(
          `[Polyspec] Failed to evaluate condition: "${expression}"`,
          `\n  Path: ${pathToString(context.currentPath)}`,
          `\n  Error:`,
          error
        );
  ...
  private evaluateExpressionValue(expression: string, context: PathContext): unknown {
    try {
      ...
      if (this.options.debug) {
        console.warn(
          `[Polyspec] Failed to evaluate expression: "${expression}"`,
          `\n  Path: ${pathToString(context.currentPath)}`,
          `\n  Error:`,
          error
        );
