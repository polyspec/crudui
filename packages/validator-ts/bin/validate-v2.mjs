#!/usr/bin/env node
/**
 * JS v2 validate CLI — gateway subprocess (cross-check-console / compare-all).
 *
 * This is the ONLY missing v2 wrapper. It exists so the cross-check gateway
 ...
 *
 * Contract (parallel to PHP `validate-v2.php`, Go `cmd/validate-v2`, Rust
 * `src/bin/validate-v2.rs`; the gateway spawns it with spawnSync, encoding
 ...
 *   - form: the full v2 pipeline `validateV2` — compose (G5) → forbidden-scan
 *     (§6) → validate (§3 + §2 G1) of `data` (the form rows).
 *   - list: the read sister `validateListV2` (SPEC §9) — compose (columns/search
 ...
 * This is a THIN wrapper: it adds no validation logic and never touches the v1
 ...
 *
 * It loads the v2 source (TypeScript / .ts imports) through the tsx loader, which
...
import { validateV2, ComposeLoadError } from '../src/v2/validate/index.ts';
import { validateListV2 } from '../src/v2/validate-list/index.ts';
...
async function main() {
  ...
  // `data` defaults to {} (JS validateV2 `data ?? {}`). Ignored in list mode.
  ...
  let result;
  try {
    result = mode === 'list' ? validateListV2(spec, opts) : validateV2(spec, data, opts);
  } catch (e) {
    ...
    emit({ error: 'validateV2 failed: ' + (e && e.stack ? e.stack : String(e)) }, 1);
  }
