...
} from '../../validator-js/src/validate/validator.ts';
import {
  FORBIDDEN_META_KEYS,
  FORBIDDEN_META_KEY_PATTERN,
} from '../../validator-js/src/schema.ts';
import { scanForbiddenKeys } from '../../validator-js/src/forbidden-scan.ts';
...
// ---------------------------------------------------------------------------
// classification (spec/schema.md §3 — quoted, the prose single-source for placement)
// ---------------------------------------------------------------------------
...
function collectClassification(schema: SchemaDoc): DescribeResult['classification'] {
  ...
  // schema §3 B as the authority for which group each key sits in.
  ...
  return {
    meta: {
      ...
      sources: {
        ...
        ruleParamClass: 'packages/validator-js/src/validate/validator.ts',
        slots: 'schema/form-spec.schema.json (definitions)',
        forbiddenKeys: 'packages/validator-js/src/schema.ts (FORBIDDEN_META_KEYS)',
        forbiddenScan: 'packages/validator-js/src/forbidden-scan.ts',
