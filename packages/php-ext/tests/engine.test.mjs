...
export function compileAndRunEngineFixture({ root, directory, source, sources, name,
  ...
  return import('node:fs').then(({ writeFileSync }) => {
    ...
    const compile = spawnSync(process.env.CC ?? 'cc', [
      ...
      ...sources.map(extensionSource), fixtureSource, '-o', executable,
    ], { encoding: 'utf8' });
...
function sourceForFormState() {
  ...
  fixture.lines.push(
    ...
    '  if (result.error) return 7; ps_value_free(result.value);',
    '  args = ps_array_value();',
    '  push(args, ps_string_value("companies.row_a.stores"));',
    '  push(args, ps_string_value("__0000000000042__"));',
    '  push(args, ps_int_value(0));',
    '  result = ps_form_apply(form, 6, args); ps_value_free(args);',
    '  if (result.error) return 8; ps_value_free(result.value);',
    '  result = ps_form_read(form, 1);',
    ...
    '  if (result.error) return 14; ps_value_free(result.value);',
    '  args = ps_array_value(); push(args, ps_string_value("companies.row_a.name"));',
