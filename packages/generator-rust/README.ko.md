...
레코드 데이터를 바인딩하고, 폼 인스턴스를 관리하고, 폼과 목록 HTML을 렌더링합니다.
...
[예제](examples/form.rs)는 중첩 폼을 컴파일하고 직렬화한 후 회사와 매장 키가 있는
데이터를 주입하고, 초기 HTML과 주입 HTML을 비교하고, 레코드를 검증하고, 완전한
HTML 문서를 출력합니다. 생성과 검증은 Rust에서 실행합니다. 브라우저 이벤트 연동과
HTTP 저장은 별도 검사 대상입니다.
