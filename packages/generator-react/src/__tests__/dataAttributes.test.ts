...
} from '../legacy/utils/dataAttributes';
import { isUniqueKey, extractUniqueKeys } from '../legacy/utils/path';
...
describe('getLegacyDataAttributes', () => {
  it('emits data-rule-name, data-name, data-default', () => {
    const attrs = getLegacyDataAttributes({ type: 'text', default: 'x' }, 'basic.name');
  ...
  it('emits empty data-default when no default is set', () => {
    const attrs = getLegacyDataAttributes({ type: 'text' }, 'name');
