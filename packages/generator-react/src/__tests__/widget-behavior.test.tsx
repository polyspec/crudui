...
it('executes handlers, updates values and retains the existing row and language containers', () => {
  ...
  try {
    ...
    expect(Array.from(date.parentElement!.children, child => child.tagName)).toEqual(['INPUT', 'BUTTON', 'BUTTON', 'BUTTON', 'BUTTON', 'BUTTON']);
    for (const language of view.container.querySelectorAll('.lang-child')) {
      expect(Array.from(language.children, child => child.tagName)).toEqual(['SPAN', 'INPUT']);
    }
    ...
    expect(view.container.querySelectorAll('[onchange]')).toHaveLength(4);
    fireEvent.click(view.container.querySelector('[data-field-path="dates"] .btn-plus')!);
...
it('preserves exact injected HTML and focus when adding a row beside a raw editor', () => {
  ...
  try {
    ...
    const button = note.parentElement!.querySelector('.btn-plus')!;
