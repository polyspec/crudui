...
export function getLegacyDataAttributes(
  ...
  // data-rule-name: field name only (matching original Legacy behavior)
  // Original Legacy uses just the field name, not the full path
  const fieldName = path.includes('.') ? path.split('.').pop()! : path;
  attributes['data-rule-name'] = fieldName;

  // data-name: field name for error messages (matching original Legacy)
  // Original Legacy uses the field name, not the label
