/**
 * v2 SSR renderer (REFERENCE) — field → HTML.
 *
 ...
 * normalization. What changes from v1 is the SOURCE of every appearance/visibility
 * decision: v1 read scattered meta keys (class/style/group_class/wrapper_class/
 * display_switch/display_target/legacyLang); v2 reads ONLY the `design` slot
 * (per-node appearance + `show`) and the `multiple`/`lang`/`group` buckets. No v1
