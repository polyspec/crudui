 ...
 * Legacy envelope (fieldHtml.ts REGISTRY); APPEARANCE is taken from the
 * resolved `design` node map (main node class/style, prepend node), and CONTENT
 * (placeholder/prepend/append) from `t()` over LocalizedText. NO legacy meta key
 * (element_class/prepend_class/style) is read here.
 *
