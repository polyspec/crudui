/**
 * CRUDUI SSR renderer (REFERENCE) — field → HTML.
 *
 ...
 * normalization. What changes from legacy is the SOURCE of every appearance/visibility
 * decision: legacy read scattered meta keys (class/style/group_class/wrapper_class/
 * display_switch/display_target/legacyLang); CRUDUI reads ONLY the `design` slot
 * (per-node appearance + `show`) and the `multiple`/`lang`/`group` buckets. No legacy
 * meta key and no `eval` are used.
 *
