 ...
 * - LargeForm.yml (1,318 lines) full validation
...
/**
 * Generate sample form data for LargeForm.yml
 */
function generateLargeFormData(withErrors: boolean = false): Record<string, unknown> {
  return {
    common: {
      is_close: 0,
      is_display: 1,
...
export async function runValidationBenchmarks(): Promise<void> {
  // Load LargeForm.yml spec
  let productNftSpec: Spec;
  try {
    productNftSpec = loadSpec('LargeForm.yml');
    console.log('\n  Loaded LargeForm.yml successfully');
  } catch (error) {
    console.error('\n  Failed to load LargeForm.yml:', error);
  ...
  // Generate test data
  const validData = generateLargeFormData(false);
  const invalidData = generateLargeFormData(true);
...
/**
 * Create a sample spec for testing when LargeForm.yml is not available
 */
