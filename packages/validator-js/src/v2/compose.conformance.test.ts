/**
 * v2 composition-engine conformance — JS reference verification.
 *
...
describe('v2 compose — success cases reproduce the expanded single spec', () => {
...
describe('v2 compose — unresolved composition is a LOAD ERROR, never valid:true', () => {
  for (const c of cases.filter((x) => x.expectError)) {
    test(c.name, () => {
      let thrown: unknown;
      try {
...
describe('v2 compose — composition is pure pre-processing (field-layer invariant)', () => {
