/**
 * `$ref` resolution — base inheritance, resolved before anything else (SPEC-V2
 * §5; v1 ReferenceResolver.php).
 *
 ...
 *
 * v2 normalization: `$ref` is the `properties`-layer composition entry point.
 ...
 * — never `valid:true` (the v1 ProductNft.yml:873 bug).
 */
...
/** The regex that splits `(path).keys` — mirrors v1 ReferenceResolver:113. */
...
export function resolveRef(
  ...
  for (const path of paths) {
    ...
    // array_merge: later keys override earlier (v1 resolve() semantics).
    merged = { ...merged, ...resolved };
  }
  return merged;
}

/** Normalize the `$ref` value into a list of path strings (v1: scalar→[scalar]). */
...
function resolveSingleRef(
  ...
  // Path-specified form `(file.yml).a.b` (v1: leading '(').
  if (path.startsWith('(')) {
    ...
    // detectKeys = explode('.', keys) ++ ['properties'] (v1:115).
    detectKeys = [...keys.split('.'), 'properties'];
  }

  // Empty path is a format error (v1: ReferenceResolver:141).
  if (path === '') {
  ...
  // (v1 has no guard and infinite-recurses; v2 must detect — SPEC §7).
  if (visiting.has(key)) {
    throw new ComposeLoadError(
      'REF_CYCLE',
  ...
  // Descend detectKeys (v1: ReferenceResolver:129-136).
 ...
 * properties map, recursively (v1: resolve() re-runs Parser::process). The
 * resolved base is laid down first, then sibling named keys override it (v1
...
function expandNestedRefs(
  ...
  // override, keys before it are overridden by it (v1 positional array_merge).
  for (const k of Object.keys(node)) {
    if (k === '$ref') {
      // base = (earlier own keys) overlaid by ref, matching v1 order where the
