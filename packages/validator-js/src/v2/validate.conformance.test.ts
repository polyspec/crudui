/**
 * v2 validator conformance — JS reference verification.
 *
 * The shared fixture tests/fixtures/validate/cases.json is declared to be the JS
 * reference v2 validator's own output (`{ valid, errors }`, or a load-error
 * code). This test RE-VERIFIES that claim by running the real engine
 * (`validateV2` = compose → traverse → validate-slot evaluation) against the
 * same fixture the other three engines (PHP / Go / Rust) load, with type-strict
 * comparison.
 *
 * It also enforces the core invariant (SPEC-V2 §5, §7): an unresolved
 ...
 * reproduce `expected` bit-for-bit (same valid + errors[], SPEC-V2 G-B).
 *
...
function run(c: FixtureCase) {
  return validateV2(c.spec, c.data, c.files ? { files: c.files } : {});
}

describe('v2 validate — result cases reproduce { valid, errors } bit-for-bit', () => {
...
describe('v2 validate — unresolved composition is a LOAD ERROR, never valid:true', () => {
...
describe('v2 validate — every fixture case is exercised', () => {
