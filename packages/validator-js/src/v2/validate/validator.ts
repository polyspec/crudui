/**
 * v2 form validator (reference) — SPEC-V2 §2 G5→§3→§2 G1.
 *
 * The third pass of the v2 pipeline. It consumes a v2 field model (the
 * `validate`/`design`/`behavior`/`options` role slots) AFTER the compose pass
 * has expanded `$ref`/`$patch` into a single spec. It does NOT touch the v1
 ...
 * parser/PathResolver). The only v2-new logic here is: (a) reading the
 * `validate` slot instead of the v1 `rules` key, (b) evaluating a rule value
 * that is an expression OR a condition map (G1 — the condition is the value's
 * expression, never a separate `if`/`when` key), and (c) dropping the v1
 ...
 *      caller (`validateV2`) BEFORE this engine. An unresolved `$ref` throws a
...
// ---------------------------------------------------------------------------
// Rule-class tables (identical to v1 — single source of truth re-declared so
// the v2 engine never imports v1 private state, R7 isolation).
// ---------------------------------------------------------------------------
...
/** A v2 field after compose: a plain object with the role slots. */
...
/** Read the per-field custom messages (v2 keeps the v1 `messages` map). */
...
/**
 * v2 validator. Construct from a COMPOSED v2 spec (composition keys already
 * eliminated). `validateV2` (index) does the compose pass first.
 */
export class ValidatorV2 {
  private readonly properties: Record<string, ComposedField>;

  /**
   * @param composedSpec a composed v2 root spec — a group with `properties`
  ...
  // =========================================================================
  // Field traversal (SPEC-V2 §3; v1 Validator.validateProperties skeleton).
  // =========================================================================
  ...
  private validateProperties(
    ...
    for (const [propertyKey, field] of Object.entries(properties)) {
      ...
      // not exist in v2; visibility-conditioned requiredness is required:'<expr>').
  ...
  /** Whether a v2 field repeats (`multiple: true` or `multiple: { ... }`). */
  ...
  private runRule(
    ...
    const validationContext: ValidationContext = {
      ...
      // The rule reads `spec.type`/`spec.messages` only; pass the v2 field as-is.
      spec: field as unknown as FieldSpec,
   ...
   *     verbatim (SPEC-V2 §10) — never evaluated as a condition.
...
// ---------------------------------------------------------------------------
// Ternary helpers (string-split, regex-safe). Self-contained so the v2 engine
// does not import v1 private functions (R7 isolation); semantics are identical
// to the v1 Validator helpers and proven equal by the shared fixtures.
// ---------------------------------------------------------------------------
