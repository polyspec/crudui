//! Rust v2 validate CLI — stdin/stdout BOUNDARY conformance.
//!
//! The validate_v2 ENGINE is already pinned by tests/validate_conformance.rs.
//! This test owns only the CLI wrapper boundary (src/bin/validate-v2.rs):
//! serialization (stdin JSON → validate_v2 → stdout {valid,errors}), exit codes,
...
//!
//! It spawns the REAL compiled binary (Cargo's CARGO_BIN_EXE_validate-v2) exactly
...
fn run_cli(input: &str) -> CliRun {
    let bin = env!("CARGO_BIN_EXE_validate-v2");
    let mut child = Command::new(bin)
        .stdin(Stdio::piped())
        .stdout(Stdio::piped())
        .stderr(Stdio::piped())
        .spawn()
        .expect("spawn validate-v2 CLI");
    child
        .stdin
        .as_mut()
        .expect("stdin")
