//! v2 list-spec structural validation (SPEC-V2 §9) — the read sister of the
//! form-spec load path.
//!
//! `list` has NO data (rows are DB-agnostic and INJECTED, never validated here —
//! SPEC-V2 §6 R1, §9.1), so the form pipeline's DATA pass (§3+§2 G1) does NOT
...
//! shared fixture `tests/fixtures/v2-list-validity/cases.json` is the single
...
/// Options for a list-spec validation run (the form `ValidateV2Options` shape —
...
mod tests {
    use super::*;
    use crate::v2::compose::ComposeErrorCode;
    use serde_json::json;
