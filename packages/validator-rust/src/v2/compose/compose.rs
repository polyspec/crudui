...
//!
//! v1's positional `array_merge` priority is normalized here: `$ref` = base
...
pub struct ComposeOptions {
    /// Basepath for relative `$ref` resolution (v1 ReferenceResolver basepath).
    pub basepath: String,
}
...
/// the single (composition-free) properties map. Named sibling keys follow v1
...
pub fn compose_properties(
    ...
    // that v1's array_merge merge priority is load-bearing on).
...
mod order_tests {
    ...
    //! so the cross-language fixture (which uses `==`) cannot catch a reorder; v1
    ...
    use crate::v2::compose::loader::MemoryLoader;
