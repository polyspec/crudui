...
pub fn resolve_ref(
    ...
    for path in &paths {
        ...
        // array_merge: later keys override earlier (declaration order).
        for (k, v) in resolved {
            merged.insert(k, v);
        }
    }
...
/// Parse the path-specified form `(file.yml).a.b`.
...
fn resolve_single_ref(
    ...
    // (detected instead of recursing forever — SPEC §7).
...
/// properties map, recursively. The resolved base is laid down first, then sibling named keys
/// override it (declaration order: a later plain key overrides an earlier $ref).
fn expand_nested_refs(
    ...
    // override, keys before it are overridden by it (positional merge).
    for (k, v) in node {
        if k == "$ref" {
            // base = (earlier own keys) overlaid by ref, in declaration order where
