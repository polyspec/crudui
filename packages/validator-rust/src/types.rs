...
//!   거부된다(additionalProperties:false). 정규 CRUDUI 키만 인식키로 등재한다 —
...
//!
//! ## round-trip
//! 4언어가 같은 CRUDUI JSON을 파싱→재직렬화해 동일해야 한다. 선언 순서는
...
///
/// `deny_unknown_fields` 가 정규 CRUDUI 키 외의 모든 최상위 키(금지 메타키·레거시
...
mod tests {
    ...
    // 정규 CRUDUI 키 외 최상위 키(금지 메타키·레거시 이름·매직 토큰·x{key})는 모델에
