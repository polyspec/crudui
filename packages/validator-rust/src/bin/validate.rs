...
//!
//! Contract (mirrors JS `validate`, Go `validate.Validate`):
//!   stdin  : `{ "spec": {…}, "data": {…}, "files"?: {…}, "basepath"?: "…" }`
//!   stdout : `{ "valid": bool, "errors": [{ path, field, rule, message, value }] }`
//!
//! Reuses the CRUDUI pipeline verbatim (`CRUDUI::validate::validate`:
//! compose → forbidden-scan → validate). It re-implements nothing and never
//! touches the legacy model (`crate::legacy::validator`, R7 parallel run).
//!
