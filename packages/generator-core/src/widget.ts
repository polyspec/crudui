/**
 * v2 widget evaluator (framework-agnostic core) — markup 0, evaluation only.
 *
 ...
 *  - the main-node class base + resolved design.class (the v1 element_class slot),
 ...
 *
 * No v1 meta key is read; no markup string is built; eval is never called.
 */
...
/** The limepie data-attr trio as a flat bag. */
 ...
 * no v1 event→onchange transform). `behavior: false/true` nullifies the slot.
 */
...
const search: Evaluator = (ctx) => {
  ...
  // Original (v1-parity) quirk: absent option → undefined !== '' → true.
