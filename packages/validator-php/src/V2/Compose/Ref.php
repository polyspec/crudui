 ...
 *
 * v2 normalization: $ref is the properties-layer composition entry point. The
 ...
 * valid:true (the v1 ProductNft.yml:873 bug).
 */
final class Ref
{
     ...
     * map, recursively (v1: resolve() re-runs Parser::process). The resolved base
     * is laid down first, then sibling named keys override it (v1 array_merge
    ...
    private static function expandNested(
        ...
        // after it override, keys before it are overridden by it (v1 positional
        // array_merge).
        foreach ($node as $k => $v) {
            if ($k === '$ref') {
                // base = (earlier own keys) overlaid by ref, matching v1 order
