 ...
 * the v2 engine 100%: expression / i18n / design / composition ($ref/$patch) /
 ...
 * CellFormat (anyOf) — are NOT this engine's job. The four-language v2 engines do
...
final class ListValidate
{
     ...
     * injected).
     *
     * @param array<string, mixed> $spec a v2 list root spec ({ columns, search?, sort?, pagination?, actions?, empty?, design?, $ref?, $patch? }; columns may use composition)
    ...
    public static function run(
        ...
        // base file exposes its list under a properties layer (the v1 detectKey
     ...
     * The base file is resolved through Ref::resolve (the v1 detectKey
    ...
    private static function composeListRoot(array $spec, FileLoader $loader, string $basepath): array
    {
        ...
        foreach ($spec as $k => $v) {
            if ($k === '$ref') {
                // base = (earlier own keys) overlaid by ref (v1 declaration order).
