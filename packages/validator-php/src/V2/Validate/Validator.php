 ...
 *       required:'<expr>').
 *
 * The v1 rule instances return a bool and skip nothing on empty values (the v1
 ...
 * + per-field `messages` override) exactly as the v1 Validator does. The shared
...
final class Validator
{
    ...
    /**
     * `match`/`pattern` carry a regex string preserved verbatim (SPEC-V2 §10: a
     ...
     * string, or a static value→label map, SPEC-V2 §2 G3). The param is data, NOT
    ...
    /**
     * Default error messages per rule (identical to the v1 Validator table). The
    ...
    /** @var array<string, RuleInterface> reused v1 rule instances */
    private array $rules;
    ...
    /**
     * @param array<string, mixed> $composedSpec a composed v2 root spec — a group
    ...
    /** Register the reused v1 rule instances. `pattern`/`match` share one. */
    ...
    private function validateProperties(
        ...
        foreach ($properties as $propertyKey => $field) {
            ...
            // do not exist in v2; visibility-conditioned requiredness is
            // required:'<expr>').
    ...
    private function runRule(
        ...
        // Central empty-skip (v1 Validator::applyRule). The reused v1 rule
        ...
        // unique with a filter/field-reference param needs the v2 expression
        // engine and the array path form, which the v1 rule lacks. Run the v2
    ...
    // =========================================================================
    // unique (v2 native; JS rules/unique parity — filter condition + paths).
    // =========================================================================
     ...
     * and nested ternaries (JS findTernaryOperator / PHP v1 ConditionParser).
     */
     ...
     * / PHP v1 ConditionParser::parseValue for scalars). A regex-pattern branch
