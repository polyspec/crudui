 ...
 *   anywhere in a v2 field — not at root, not one level below any slot/bucket.
 *
 * Recognition vs translation. Only canonical v2 names are recognition keys
 ...
 * for the legacy->v2 translator and are NEVER mixed into the recognition sets.
 *
 ...
 *
 * Round-trip. parse() reads a v2 JSON document into an ordered associative array
...
final class FieldSpec
{
     ...
     * its role. A v2 field accepts these keys and nothing else; the schema
     ...
     * here are CANONICAL v2 names only — no legacy spellings, no magic tokens.
    ...
    /**
     * Legacy->v2 translation table for the dependency buckets. For the legacy
     ...
     * Each entry: bucket => [ legacy_name => canonical_v2_name ].
     *
    ...
    /**
     * Condition-only and legacy meta keys that MUST NOT appear ANYWHERE in a v2
    ...
    /**
     * Whether a key is forbidden in a v2 field at ANY depth.
     */
    ...
    /**
     * Canonical v2 name for a legacy dependency-bucket key, or null when the
    ...
    /**
     * Whether a content value is a valid v2 content shape (SPEC §2 G3): a plain
    ...
    /**
     * Deep-validate a decoded v2 field. Returns the list of violations (empty =
    ...
    /**
     * Parse a v2 JSON document into an ordered associative array for the analysis
    ...
    /**
     * Serialize a decoded v2 field (associative-array form) back to canonical
    ...
    /**
     * Parse a v2 JSON document into an object tree (stdClass for objects, list
    ...
    /**
     * Serialize a v2 object tree back to canonical JSON, preserving key order and
