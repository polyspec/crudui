...
import { ComposeLoadError, UnsupportedFieldTypeError } from '../src/v2/index.ts';
...
// concrete markup; if any of these names reaches the RAW SSR output, the eval/v1
...
describe('v2 render — eval is never used (no v1 condition meta keys leak)', () => {
  ...
  test('multiple-group-rows — real data row id survives unmasked (G4)', async () => {
    ...
    expect(await renderSSR(c)).toContain('data-uniqid="p1"');
  });
});
