/**
 * v2 Vue list SSR entry — render a composed list-spec through Vue's own renderer.
 *
 ...
 * core; the Vue adapter (ListV2) builds a REAL vnode tree (table/cards) from that
...
/**
 * Render a v2 list-spec to its SSR HTML string via Vue 3's server renderer.
 ...
 * never silent).
 */
export async function renderListV2SSR(
  ...
  // Genuine Vue 3 SSR of a REAL vnode tree (ListV2 → table/cards → cells).
  const { createSSRApp } = await import('vue');
  const { renderToString } = await import('@vue/server-renderer');

  const app = createSSRApp({
    render: () => ListV2(vm, layout ?? 'table'),
  });
