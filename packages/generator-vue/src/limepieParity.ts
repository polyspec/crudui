 ...
 *
 * Framework-independent port (adapted from generator-react). The golden
 * fixtures (tests/fixtures/golden-html) are the single source of truth. Do
 ...
 * Accepts ordered pair form ([[key, label], ...]), plain arrays, and objects.
 */
...
/** The Limepie validation attribute trio (data-name/rule-name/default). */
export function limepieDataAttrs(
  spec: ReactFieldSpec,
  path: string
...
/**
 * Parse an inline CSS string into a canonical style STRING (or undefined for
 * empty input so the attribute is omitted). The string form is emitted
 * verbatim by Vue h(); normalize.js canonicalizes declarations anyway.
 */
export function parseStyleString(style: unknown): string | undefined {
...
/** Deterministic stand-in for PHP genRandomString(5) used in choice ids. */
