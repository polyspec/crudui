 ...
 * SAME surfaces the CRUDUI validator consumes (validator-js/src/parser/*). This file
 * is a thin dispatcher that ports the validator's `resolveRuleValue` /
 * `resolveConditionMap` / `tryEvaluateTernary` patterns
 * (validator-js/src/validate/validator.ts:519,543,578) so the generator
 ...
 * 3-framework idempotence is anchored on tests/fixtures/expr/cases.json. The Vue
 * generator reuses the SAME exported functions as React/Svelte (no fork).
 *
