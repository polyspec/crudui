 ...
 * primitives. These reproduce the verified Legacy envelope conventions
 * (legacyParity.ts) that the Svelte/React reference renders use, so the CRUDUI
 * envelope is bit-identical to the proven legacy output after normalization. NO legacy
 * meta key is read here; these are pure structural primitives (escaping,
 * name/id derivation, php-string casts, deterministic uniqid, value lookup).
 */
