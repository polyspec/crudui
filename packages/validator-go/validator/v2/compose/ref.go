...
// v1 ReferenceResolver.php). Port of ref.ts.
//
...
// pathSpecRE splits (path).keys — mirrors v1 ReferenceResolver:113 and the JS
// /^\((?<path>.*?)\)\.(?<keys>.*)$/.
...
// resolved properties map, recursively (v1: resolve() re-runs Parser::process).
// The resolved base is laid down first, then sibling named keys override it (v1
