...
// Composition orchestrator (SPEC-V2 §5, G5) — the parser's FIRST pass. Port of
// compose.ts.
//
...
// keys follow v1 declaration order — a key declared after $ref overrides the
...
func ComposeSpec(spec *OMap, loader FileLoader, opts ComposeOptions) (*OMap, error) {
	...
	if spec.Has("$ref") || spec.Has("$patch") {
		...
		for _, k := range spec.Keys() {
			...
			switch k {
			case "$ref":
				// Field-level $ref resolves a file's properties layer too (v1 detectKey).
