// Package compose is the Go port of the v2 composition engine (SPEC-V2 §5, G5).
//
...
// ERROR (ComposeLoadError) — never valid:true (the v1 ProductNft.yml:873 bug).
// v2-NEW only: this never touches the v1 model or loader (R7 parallel run).
//
// Byte-for-byte parity with the JS reference (validator-js/src/v2/compose). Both
...
const (
	// RefFileNotFound: $ref points at a file that does not exist (v1 yml_parse_file fail).
	...
	// RefCycle: $ref cycle detected (A→B→A); v1 would infinite-recurse.
