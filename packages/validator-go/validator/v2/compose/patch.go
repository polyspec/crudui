...
// Port of patch.ts. Absorbs the legacy v1 directives
...
// the existing leaf and the new value are plain objects, DEEP-MERGE (v1 $merge);
...
// mergeValue is the v1 deep-merge leaf rule (drupal_array_merge_deep_array): both
...
// removeNested is the nested-map remove (v1 arr::remove): for each key, recurse
// when both the target and the removal spec are objects, else unset the key. A
// missing key is tolerated here (v1 arr::remove silently unsets), unlike the
// array-path form.
