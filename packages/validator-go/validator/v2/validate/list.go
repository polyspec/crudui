...
// v2 list-spec validation entry point (SPEC-V2 §9). The READ sister of the form
...
// (§6) — and NOTHING ELSE.
//
// Why only two passes (SPEC-V2 §9.1): a list declares the read structure
...
// validate pass (§3 + §2 G1, index.go ValidatorV2) evaluates field validate-slot
...
// schema/polyspec-v2.schema.json #/definitions/List, exercised by
...
func ValidateList(spec *compose.OMap, opts Options) (ValidationResult, error) {
	...
	// tree.
	if scanErr := v2.ScanForbiddenKeys(composed, []string{"list"}); scanErr != nil {
