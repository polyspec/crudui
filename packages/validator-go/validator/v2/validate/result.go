// Package validate is the Go v2 form validator (SPEC-V2 §2 G5 → §3 → §2 G1) —
// the THIRD pass of the v2 pipeline.
//
// It consumes a v2 field model (the validate / design / behavior / options role
// slots) AFTER the compose pass (validator-go/validator/v2/compose) has expanded
...
//	    engine (ValidateV2 in index.go). An unresolved $ref is a *ComposeLoadError
//	    there (never valid:true) — the v1 ProductNft.yml:873 gap closed.
...
// ValidationError is one validation failure. Field shape is identical to the v1
// validator (path / field / rule / message / value) so the 4-language idempotence
// comparison (SPEC-V2 G-B) holds with the same (path, rule, message).
