...
// v2 validator traversal + conditional rule-value resolution (SPEC-V2 §3 + §2 G1).
//
...
// JS ValidatorV2 (validator-ts/src/v2/validate/validator.ts) field for field; the
...
// exist in v2; visibility-conditioned requiredness is required:'<expr>'). design
// .show does NOT skip validation (SPEC-V2 R1 show/validate separation).
...
import (
	...
	"github.com/polyspec/polyspec/packages/validator-go/validator/v2/expr"
)
...
// regexParamRules carry a regex string preserved verbatim (SPEC-V2 §10).
...
// static value→label map, SPEC-V2 §2 G3). The param is data, NOT a condition
...
// ValidatorV2 validates data against a composed v2 spec. Construct via NewValidator
...
func NewValidator(properties *compose.OMap) *ValidatorV2 {
	if properties == nil {
		properties = compose.NewOMap()
	}
	return &ValidatorV2{properties: properties}
}

// Validate validates data against the composed v2 spec.
func (v *ValidatorV2) Validate(data map[string]any) ValidationResult {
	if data == nil {
		data = map[string]any{}
	}
...
// validateProperties recurses a properties map (SPEC-V2 §3; JS validateProperties).
func (v *ValidatorV2) validateProperties(properties *compose.OMap, data map[string]any, currentPath []string, allData map[string]any, errors *[]ValidationError) {
...
func (v *ValidatorV2) validateElementRules(field *compose.OMap, value any, itemPath []string, allData map[string]any, errors *[]ValidationError) {
	messages := fieldMessages(field)
	rules := normalizeValidateSlot(field)
...
func (v *ValidatorV2) validateFieldRules(field *compose.OMap, value any, fieldPath []string, allData map[string]any, errors *[]ValidationError) {
	messages := fieldMessages(field)
	rules := normalizeValidateSlot(field)
...
func (v *ValidatorV2) runImplicitNumber(field *compose.OMap, rules *compose.OMap, value any, path []string, messages map[string]string, allData map[string]any, errors *[]ValidationError) bool {
...
func (v *ValidatorV2) runRule(ruleName string, ruleValue any, value any, path []string, messages map[string]string, allData map[string]any) (string, bool) {
...
func (v *ValidatorV2) resolveRuleValue(ruleName string, ruleValue any, path []string, allData map[string]any) any {
...
func (v *ValidatorV2) resolveConditionMap(m *compose.OMap, path []string, allData map[string]any) any {
...
func (v *ValidatorV2) tryEvaluateTernary(expression string, path []string, allData map[string]any) (bool, any) {
...
func (v *ValidatorV2) evaluateCondition(expression string, path []string, allData map[string]any) bool {
...
func (v *ValidatorV2) evaluateExpressionValue(expression string, path []string, allData map[string]any) any {
