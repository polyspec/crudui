...
## 2026-09-16 — 폼 푸터의 장식용 테두리 제거

폼 푸터는 고정 위치·높이·컨트롤을 유지하면서 장식용 상단 테두리를 표시하지 않습니다.
행은 카드 하단을 유지해 빈 공간으로 끝나지 않게 하며, 행 헤더에는 행 하단과 중복될 수
있는 하단 테두리를 두지 않습니다. 브라우저 스타일 검사는 페이지·스크롤 박스·프레임
호스트에서 푸터와 행 경계 계약을 검사합니다.
