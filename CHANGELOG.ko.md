...
브라우저 검증기가 검증 입력을 `Control+A`로 선택한 뒤 값을 삭제합니다. Chromium의 플랫폼에
따라 달라지는 다중 클릭 선택 동작에 더 이상 의존하지 않습니다.
...
- `tests/fixtures/specs/ProductNft.yml`에는 중복 YAML 키가 두 곳 있었고 벤치 고정 데이터 생성기가 `uniqueKeys: false`로
