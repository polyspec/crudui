...
CI와 Node.js 컨테이너 단계는 정확한 패치 릴리스를 고정하지 않고 다음 LTS 릴리스
계열인 Node.js 26을 선택합니다. CI는 현재 안정 npm 릴리스를 설치합니다. 런타임
정책 검사는 `.node-version`, CI, 컨테이너 정의에 동일한 짝수 Node.js 계열을
요구합니다. 패키지 잠금 파일은 해석한 패키지 버전을 계속 기록합니다.
