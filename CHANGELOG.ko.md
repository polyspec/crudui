...
## 2026-09-16 — sticky 행 구분선 잔여 1픽셀 제거

행 헤더는 일반 위치에서 구분선을 유지하지만 sticky 헤더가 고정된 동안에는 구분선을
숨깁니다. 이로써 이동하는 행 경계에 1픽셀이 남지 않으며, 접힌 행도 구분선을 숨깁니다.
브라우저 검사는 페이지·스크롤 박스·프레임 호스트를 확인합니다.
