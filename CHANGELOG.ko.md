...
고정 행 카드의 위쪽 선은 고정되는 박스인 헤더 안에서 그려지고, 헤더가 고정되는 동안에는
사라져 바로 위 헤더의 선이 이음매가 됩니다. 고정선은 깊이에 헤더 높이를 곱한 값에서 행
테두리 한 겹을 뺀 값이므로, 고정된 헤더는 위 헤더의 선 위에 놓이고 고정선에 닿는 행도 자기
선을 같은 위치에 놓습니다. 이전에는 이 테두리가 행에 있어 행과 함께 스크롤했고, 그 선의 한
픽셀 아래에 서면서 행이 고정선에 닿는 위치에서 이음매가 두 겹으로 그려졌습니다.
