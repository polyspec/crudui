...
넣은 뒤 선택한 프레임워크가 같은 템플릿과 데이터로 폼을 넘겨받습니다. 넘겨받은 뒤에도 폼
마크업은 브라우저 바인딩이 쓰는 상태(`data-crudui-stuck`, `data-crudui-current`, 끝 행 길이)를
제외하고 바뀌지 않아야 합니다. CSR 열(`initialization=csr`)은 데이터 없이 폼을 마운트한 뒤
